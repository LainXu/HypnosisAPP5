...
// 不再处理 MC能量、主角可疑度、角色警戒度，也不维护 _当前周几/_当前日程/课程表；这些规则分别由世界书和前端负责。
...
- 开场没有{{user}}与三位女角色的直接互动，但她们都看见或注意到{{user}}的身份形象，因此必须根据\`/系统/user身份\`与各自性格给出轻微第一印象数值变化：好感度、警戒度、服从度可按-3、-1、+1、+3等小幅变化独立判断；困难/极难/你是学生？身份通常更容易提高警戒或降低好感，简单/普通身份通常更容易获得低幅好感或低警戒。不要因为“没有对话”就让三人全部数值不变。
...
叙事边界:
- 身份选择不是催眠命令、校规或道具制造，不会产生催眠效果或校规变化；但固定开场里的旁观、第一眼印象和班级气氛本身可以造成爱丽莎、深雪、夏美的轻微好感度、警戒度、服从度变化，必须按身份难度和角色人设写入变量。
...
- 催眠APP、领取任务、完成成就、购买VIP、补给、库存、日历、删除角色、新增任务、打工、邂逅随机桃花运、邂逅商店、特殊地点准入证、课程表魔改券、地图/学校地图地点建议和申请/废止校规等操作都按本规则结算；邂逅指定角色包/单独角色仅浏览不结算；催眠命令白名单与费用见[mvu_update]催眠命令计费规则，校规的作用范围与写入位置见[mvu_update]校规规则，课程表魔改券见[mvu_update]课程表魔改券规则。
...
- 邂逅中的指定角色包和单独角色已改为仅浏览资料，不再购买、不扣星光点、不创建角色变量、不写入世界书。只有随机桃花运仍消耗6星光点，并由前端随机抽中角色、创建初始变量、把角色变量/人设/可选好感链写入角色卡世界书；AI只需要按本轮邂逅提示安排桃花运剧情，不再创建、重建或补写角色/世界书。
...
- 邂逅APP的\`角色包已使用\`现在只应由随机桃花运产生，表示前端已经完成随机购买确认、扣除\`/系统/星光点\`、创建初始\`/角色\`变量、缓存角色图片，并尝试把命中角色的变量/人设/可选好感链条目写入角色卡世界书对应位置；指定角色包/单独角色浏览不会产生此操作。随机桃花运由前端先从带世界书内容且角色卡世界书未导入的角色中随机抽中1名，AI不得重新随机或替换命中角色。世界书插入不可撤销，若需要撤销只能由用户之后手动删除；变量可随楼层回滚。同一轮最多处理一个随机桃花运，若异常出现多条，只处理最早一条。AI收到该操作时不要重复扣星光点，不要重复插入世界书，也不要创建、重建或补写角色/世界书；只根据角色包信息、整体邂逅提示、已写入世界书和当前剧情安排对应角色登场，并在后续按剧情变化更新\`/角色\`变量。已存在角色只补缺失字段或在正文说明冲突。
...
- 角色根字段\`事件记录\`是前端只读维护的6位字符串，默认\`000000\`；前五位表示该角色事件1到事件5是否已触发，第六位表示好感度>=200且服从度>=100时由用户在前端弹窗自定义的第六事件是否已触发。事件1触发显示为\`100000\`，事件2触发把第二位改为1，依此类推。用户在人物档案事件页选择事件时，前端会直接写入对应位并锁定暂存；AI只根据本轮\`触发角色事件\`操作生成对应事件剧情，不得手写、补写、回退、清空或自行replace这个字符串。触发事件的回复末尾必须额外输出完整闭合块：\`<人物档案事件记录>\`开头，字段包含\`角色名\`、\`事件序号\`、\`标题\`、\`概要\`、\`关键场面\`、\`关系变化\`和\`后续钩子\`，最后必须单独一行写\`</人物档案事件记录>\`；缺少闭合标签视为格式错误。该块只供前端本地保存回忆摘要，不写入MVU变量。没有前端事件操作时，不要自行触发编号事件。
...
- \`事件记录\`是角色个人事件位图，格式固定6位\`0/1\`字符串，由前端只读维护；前五位对应好感链事件，第六位对应好感度>=200且服从度>=100时由用户弹窗自定义的事件。用户在人物档案事件页选择事件时前端直接改对应位并锁定暂存。AI只根据该操作写事件剧情，不得手写、补写、回退、清空或自行replace事件记录；触发事件的回复末尾必须输出完整闭合的\`<人物档案事件记录>\`块，字段包含角色名、事件序号、标题、概要、关键场面、关系变化和后续钩子，最后必须单独一行写\`</人物档案事件记录>\`，供前端本地保存为回忆摘要，不写入MVU变量；没有前端事件操作时，不要自行触发编号事件。
...
适用范围: 邂逅角色包浏览、单独角色浏览、随机桃花运、邂逅商店、星光点兑换券、课程表魔改券、校规修改券兑换。

规则:
- 邂逅中的指定角色包和单独角色仅作为资料浏览、定制、导入导出和世界书管理入口，不消耗星光点，不创建角色变量，不把角色条目写入世界书。
- 随机桃花运消耗6星光点。若星光点不足，操作失败，不导入、不登场。
- 星光点在邂逅里不是普通购物货币，而是购买“桃花运”的代价：{{user}}只能在APP界面主动购买随机桃花运；剧情中的{{user}}只知道自己主动购买了一次桃花运，不知道具体会遇到谁、遇到几个人、对方来自哪个角色包。AI必须把登场写成APP暗中安排的偶遇/桃花运，而不是{{user}}精准点名召唤角色。
- 随机桃花运由前端从带世界书内容且角色卡世界书未导入的角色中随机抽中1名，然后扣星光点、创建初始变量，并把该角色的变量、人设和可选好感链条目写入角色卡世界书对应位置。AI收到“随机命中角色”后不得重新抽、不得换人、不得把未命中的角色加入。
- 星光点是APP内部货币，邂逅登场角色不知道星光点、不能支付星光点、不能返还星光点，也不能把自己的人脉/金钱/资源兑换成星光点；若角色愿意支持{{user}}，只能提供剧情资源、零花钱、物品、场地、人脉或情报。
- 角色包已使用现在只应对应随机桃花运命中的角色：前端已经完成随机购买确认、扣除/系统/星光点、创建初始/角色变量、缓存角色图片，并尝试把命中角色世界书内容写入角色卡世界书。指定角色包/单独角色浏览不会产生这个操作。
...
- 若按开始时间规则无新增buff，不要因本次打工新增或清空buff；已有buff只会在/系统/_buff结束时间到达后由前端清空。
...
- 每个阈值事件只发生一次。/角色/西园寺爱丽莎/事件记录由前端只读维护，六位中前五位从左到右对应40、70、100、140、200；第六位是好感度>=200且服从度>=100时由用户弹窗自定义的第六事件。某位为1表示该事件已被前端占位触发，不要重复触发。高阈值事件已发生时，低阈值只可作为回忆带过。
...
const resourceBlock = `    MC能量:
    ...
    星光点:
      ...
      check:
        - 只有前端领取成就/任务奖励、监控派遣结算、星光点兑换券兑换等明确APP系统来源成功时才增加；购买VIP3-6附加费用、邂逅随机桃花运、邂逅商店兑换、购买特殊地点准入证、购买课程表魔改券和废止初始校规等成功时减少。指定角色包/单独角色浏览不消耗星光点。
    ...
    _buff:
      ...
      check:
        ...
        - 本字段为前端只读维护字段，AI不要手写。打工可能由前端写入\`社会的蔑视\`、\`无精打采\`或\`全盛出击\`；所有打工buff从打工开始时间持续到前端给出的\`_buff结束时间\`。是否仍在持续期内以本次打工选择的开始时间判断：若开始时间早于\`_buff结束时间\`，{{user}}精力不足，不能开始新的打工；若开始时间已到达或晚于\`_buff结束时间\`，前端会先清空旧buff再结算新打工。
    ...
    _buff结束时间:
      ...
      check:
        ...
        - 打工成功产生\`社会的蔑视\`、\`无精打采\`或\`全盛出击\`时，由前端写入本轮操作给出的\`_buff结束时间\`。
...
function patchCard(card) {
  ...
  patchEntry(entries, "[mvu_plot]催眠指导", (content) => content
    ...
    .replace(
      ...
      "所有涉及花费的催眠APP功能在生效前必须逐项检查余额：`系统.MC能量`支付启动/追加催眠和催眠命令费用；`系统.持有零花钱`支付购买VIP、补充MC能量、提升MC能量上限等金钱费用；`系统.星光点`支付VIP3-6附加星光点、邂逅随机桃花运、邂逅商店校规修改券兑换、邂逅商店特殊地点准入证购买、课程表魔改券购买、废止初始校规等星光点费用；`系统.持有物品`里的校规修改券只支付申请/发布新校规，课程表魔改券只支付课程表APP保存当天剩余课程中的一节修改，特殊地点准入证只提供对应地点进入资格。指定角色包/单独角色浏览不消耗星光点。购买VIP还必须逐级满足前置等级。余额不足或前置不足则该功能失败，不产生催眠效果，也不得扣成负数。"
    ));
  ...
  patchEntry(entries, "[mvu_update]变量更新格式", (content) => {
    ...
    next = next.replace(
      /\n-\s*时间\/地点\/事件更新要求：[^\n]*日期和星期写在`\/系统\/当前日期`。[^\n]*/g,
        "\n- 时间/地点/事件更新要求：凡正文、结算摘要或用户输入中出现明确时间推进、转场、当前位置或当前事件变化，必须在同一次<update>里写`/系统/当前时间`、`/系统/当前地点`、`/系统/当前事件`的最终值；当前时间只写`HH:MM`，`/系统/当前日期`只写日期（如`4月9日`），不要把星期写进当前日期。`/系统/_当前周几`、`/系统/_当前日程`、`/系统/_当前特殊日期`和`/系统/_课程表`是前端只读同步字段，AI不要手写；前端会按当前日期/时间每层同步一次。上述系统时间/地点/事件字段已存在用`replace`，字段不存在先用`add`；这条`add`许可只限这些系统字段，不适用于`/角色`。只写英文System variables摘要、Time passed摘要或裸JSON Patch数组都不算完成变量更新。"
    );
  ...
  patchEntry(entries, "[mvu_plot]人物列表", (content) => content
    ...
    .replaceAll(
      "`事件记录:\"000000\"`、核心数值",
      "`事件记录:\"000000\"`、`至关重要记忆:\"\"`、核心数值"
    )
    .replaceAll(
      "`事件记录`为6位0/1字符串，前五位记录角色事件1-5是否已触发，第六位记录好感度>=200且服从度>=100的用户自定义事件是否已触发；`工作价值`",
      "`事件记录`为6位0/1字符串，前五位记录角色事件1-5是否已触发，第六位记录好感度>=200且服从度>=100的用户自定义事件是否已触发；`至关重要记忆`为前端只读当前回忆焦点，默认空字符串，只由人物档案回忆按钮写入；`工作价值`"
    )
    .replace(
    ...
    .replace(
      ...
      "变量结构需包含`档案`(照片、姓名、年龄、社团/职业、身高、体重、三围、头发、面部、上衣、下衣)、`心理`(此刻想法)、`是否派遣中:false`、`工作价值`、`绰号`(默认空字符串)、`绰号已认可:false`、`事件记录:\"000000\"`、`至关重要记忆:\"\"`、核心数值、敏感度、次数、临时/永久催眠效果；`事件记录`为6位0/1字符串，前五位记录角色事件1-5是否已触发，第六位记录好感度>=200且服从度>=100的用户自定义事件是否已触发；`至关重要记忆`为前端只读当前回忆焦点，默认空字符串，只由人物档案回忆按钮写入；`工作价值`单位为星光点/日，按人设、身份、能力、资源、社会价值、派遣变现潜力和剧情定位生成；生成后固定，后续剧情变化不重算工作价值；核心数值范围-200到200，部位敏感度范围0到1000；其中上衣/下衣分别记录上半身/下半身当前可见状态，包含衣物与未被衣物覆盖的肌肤，必要时可保留NSFW细节。"
    )
    .replace(
