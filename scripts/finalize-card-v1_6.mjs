...
- 之后若收到前端锁定的\`打工偶遇（剧情进行中）\`提醒，先处理本轮用户与偶遇角色的互动，再把剧情收束到\`预计结束时间\`；这是收束剧情，不是补发工资、补加社畜值或补写buff。
...
function patchCard(card) {
  ...
  patchEntry(entries, "[mvu_plot]催眠指导", (content) => content
    ...
    .replace(
      ...
      "所有涉及花费的催眠APP功能在生效前必须逐项检查余额：`系统.MC能量`支付启动/追加催眠和催眠命令费用；`系统.持有零花钱`支付购买VIP、补充MC能量、提升MC能量上限等金钱费用；`系统.星光点`支付VIP3-6附加星光点、邂逅随机桃花运、邂逅商店校规修改券兑换、邂逅商店特殊地点准入证购买、课程表魔改券购买、废止初始校规等星光点费用；`系统.持有物品`里的校规修改券只支付申请/发布新校规，课程表魔改券只支付课程表APP进入魔改课程表编辑模式，特殊地点准入证只提供对应地点进入资格。指定角色包/单独角色浏览不消耗星光点。购买VIP还必须逐级满足前置等级。余额不足或前置不足则该功能失败，不产生催眠效果，也不得扣成负数。"
    ));
