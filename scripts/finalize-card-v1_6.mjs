...
- 静态/初始任务接取由前端直接写入\`/任务/固定任务名\`并锁定本轮事实；AI只承认已接取，不得新增、改名或重写固定任务内容。静态任务只有已存在于\`/任务\`时才可按剧情结算完成。新增任务表示前端按\`/系统/当前日期\`和当前聊天名每日固定roll一个当前角色作为“今天任务目标”，同一日期同一聊天名不会变化；用户只点击接受目标，不提前知道任务内容，最多同时3个进行中任务。
...
- 催眠APP、领取任务、完成成就、购买VIP、补给、库存、日历、删除角色、新增任务、打工、邂逅随机桃花运、邂逅指定角色桃花运、邂逅商店、特殊地点准入证、课程表魔改券、地图/学校地图地点建议和申请/废止校规等操作都按本规则结算；邂逅指定角色包本身仅浏览不结算，包内具体角色可以作为指定角色购买；催眠命令白名单与费用见[mvu_update]催眠命令计费规则，校规的作用范围与写入位置见[mvu_update]校规规则，课程表魔改券见[mvu_update]课程表魔改券规则。
...
- 邂逅中的指定角色包本身仅浏览资料和管理世界书，不购买、不扣星光点、不创建角色变量、不写入世界书；包内具体角色和单独角色可以作为指定角色桃花运购买。随机桃花运和指定角色桃花运均消耗5星光点，并由前端创建初始变量、把角色变量/人设/可选好感链写入角色卡世界书；随机桃花运由前端随机抽中1名，指定角色桃花运则以本轮操作里的指定角色为准。AI只需要按命中/指定角色的人设、角色出场提示和当前上下文安排桃花运剧情，不再创建、重建或补写角色/世界书。
...
- 邂逅APP的\`随机桃花运已使用\`和\`角色已使用\`表示前端已经完成购买确认、扣除\`/系统/星光点\`、创建初始\`/角色\`变量、缓存角色图片，并尝试把目标角色的变量/人设/可选好感链条目写入角色卡世界书对应位置；\`角色包已使用\`只是历史兼容操作名，按同一规则读取。指定角色包本身浏览不会产生购买操作。随机桃花运由前端先从带世界书内容且角色卡世界书未导入的角色中随机抽中1名，AI不得重新随机或替换命中角色；指定角色桃花运则必须使用操作里的指定角色，不得换人。世界书插入不可撤销，若需要撤销只能由用户之后手动删除；变量可随楼层回滚。同一轮最多处理一个邂逅购买，若异常出现多条，只处理最早一条。AI收到该操作时不要重复扣星光点，不要重复插入世界书，也不要创建、重建或补写角色/世界书；只根据命中/指定角色信息、人设/角色出场提示、已写入世界书和当前剧情安排对应角色登场，并在后续按剧情变化更新\`/角色\`变量。已存在角色只补缺失字段或在正文说明冲突。
...
- \`任务\`变量保存已接/进行中任务，也可保存已经完成但尚未手动领取奖励的任务；最多3个进行中/待领取任务。固定/初始任务接取已由前端直接写入\`/任务\`并锁定事实；AI不得把固定任务当成新增任务，不得重新命名或新建同义任务。新增任务操作表示前端按\`/系统/当前日期\`和当前聊天名每日固定roll一个当前角色作为“今天任务目标”，同一日期同一聊天名不会变化；不是{{user}}主动发布、设计或提前知道的具体任务内容；若已接任务数为3则本次新增失败。
...
适用范围: 邂逅角色包浏览、单独角色浏览、指定角色桃花运、随机桃花运、邂逅商店、星光点兑换券、课程表魔改券、校规修改券兑换。

规则:
- 邂逅中的指定角色包仅作为资料浏览和世界书管理入口，不消耗星光点，不创建角色变量，不把整包角色条目写入世界书；角色包内的具体角色和单独角色可以作为指定角色桃花运购买。
- 随机桃花运和指定角色桃花运均消耗5星光点。若星光点不足，操作失败，不导入、不登场。
- 星光点在邂逅里不是普通购物货币，而是购买“桃花运”的代价：{{user}}只能在APP界面主动购买随机桃花运或指定角色桃花运。随机桃花运中，剧情里的{{user}}只知道自己购买了一次随机桃花运，不知道具体对象；指定角色桃花运中，{{user}}知道自己在APP里选择了该角色，但登场仍必须写成APP暗中安排的自然偶遇，而不是{{user}}凭空召唤角色或绕过当前地点、时间、关系限制。
- 随机桃花运由前端从带世界书内容且角色卡世界书未导入的角色中随机抽中1名；指定角色桃花运以本轮操作中的指定角色为准。前端扣星光点、创建初始变量，并把目标角色的变量、人设和可选好感链条目写入角色卡世界书对应位置。AI收到“随机命中角色”或“指定角色”后不得重新抽、不得换人、不得把未命中的角色加入。
- 星光点是APP内部货币，邂逅登场角色不知道星光点、不能支付星光点、不能返还星光点，也不能把自己的人脉/金钱/资源兑换成星光点；若角色愿意支持{{user}}，只能提供剧情资源、零花钱、物品、场地、人脉或情报。
- \`随机桃花运已使用\`和\`角色已使用\`表示前端已经完成购买确认、扣除/系统/星光点、创建初始/角色变量、缓存角色图片，并尝试把目标角色世界书内容写入角色卡世界书。\`角色包已使用\`只是历史兼容操作名，读取到时按同一逻辑处理；指定角色包本身浏览不会产生购买操作。
...
- 同一轮最多处理一个随机桃花运，若异常出现多条，只处理最早一条。
- AI收到该操作时不要重复扣星光点，不要重复插入世界书，也不要创建、重建或补写角色/世界书；只根据命中角色信息、人设/角色出场提示、已写入世界书和当前剧情安排对应角色登场，并在后续按剧情变化更新/角色变量。
- 已存在角色只补缺失字段或在正文说明冲突。
...
- 之后若收到前端锁定的\`打工偶遇（剧情进行中）\`提醒，先处理本轮用户与偶遇角色的互动；本轮应暂停在偶遇剧情，等互动完成后再把剧情收束到\`预计结束时间\`；这是收束剧情，不是补发工资、补加社畜值或补写buff。
...
const resourceBlock = `    MC能量:
    ...
    星光点:
      ...
      check:
        - 只有前端领取成就/任务奖励、监控派遣结算、星光点兑换券兑换等明确APP系统来源成功时才增加；购买VIP3-6附加费用、邂逅随机桃花运、邂逅指定角色桃花运、邂逅商店兑换、购买特殊地点准入证、购买课程表魔改券和废止初始校规等成功时减少。指定角色包本身浏览不消耗星光点。
...
const CharacterStats = z.intersection(`
  ...
  for (const key of ["校规", "任务", "临时催眠效果", "永久催眠效果"]) {
...
function patchCard(card) {
  ...
  upsertEntry(entries, {
    ...
    keys: ["邂逅", "桃花运", "随机桃花运", "随机桃花运已使用", "角色已使用", "角色包", "角色包已使用", "单独角色", "邂逅商店", "星光点兑换券", "课程表魔改券", "校规修改券"],
  ...
  patchEntry(entries, "[mvu_plot]催眠指导", (content) => content
    ...
    .replace(
      ...
      "所有涉及花费的催眠APP功能在生效前必须逐项检查余额：`系统.MC能量`支付启动/追加催眠和催眠命令费用；`系统.持有零花钱`支付购买VIP、补充MC能量、提升MC能量上限等金钱费用；`系统.星光点`支付VIP3-6附加星光点、邂逅随机桃花运、邂逅指定角色桃花运、邂逅商店校规修改券兑换、邂逅商店特殊地点准入证购买、课程表魔改券购买、废止初始校规等星光点费用；`系统.持有物品`里的校规修改券只支付申请/发布新校规，课程表魔改券只支付课程表APP保存当天剩余课程中的一节修改，特殊地点准入证只提供对应地点进入资格。指定角色包本身浏览不消耗星光点；包内具体角色和单独角色作为桃花运购买时消耗5星光点。购买VIP还必须逐级满足前置等级。余额不足或前置不足则该功能失败，不产生催眠效果，也不得扣成负数。"
    ));
