...
function normalizeWorldbookActivationModes(entries) {
  const greenEntries = [
    ...
    ["[mvu_update]特殊地点规则"],
    ["[mvu_plot]私立斋明学园设定", ["私立斋明学园", "斋明学园", "校园", "学校", "班级", "年级", "男生很少", "男学生", "二年A组", "学生比例"]],
    ["[mvu_update]金钱与星光点规则"],
    ["[mvu_update]失败行动处理规则"]
  ];
  ...
  function applyDebugFrontendStorage() {
    ...
    writeJsonStorage("hypnoos.profile.eventMemories.v1:" + encodeURIComponent(scope).slice(0, 220), {
      ...
      events: {
        ...
        "犬冢夏美": {
          "0": { title: "派遣区测试记录", summary: "夏美已在派遣区三天。", detail: "用于测试监控结算与派遣中限制。", updatedAt: Date.now() }
        }
      }
    });
...
- 校规只存放在\`/校规\`，最多5条；默认初始校规也允许被移除。若本轮学校APP操作写明\`前端处理: 已由前端直接写入变量\`，说明前端已经删除对应\`/校规/校规名\`并完成扣费，AI只承认该校规不再生效，不得再次remove、二次扣费或返还资源。未写明前端已处理的旧式删除/废止校规才由AI按下列规则结算。
...
- 废止初始默认校规（\`仪容礼仪\`、\`出勤学习\`、\`校内安全\`、\`校内风纪\`、\`环境卫生\`）必须VIP6且持有\`星光点\`至少10点；旧式AI结算成功时扣除10点\`星光点\`并remove对应校规，失败不扣费不删除。废止初始校规不消耗\`校规修改券\`。
- 删除后续由{{user}}新增的校规不需要支付代价；旧式AI结算成功只remove对应校规，不返还MC能量或金钱。
</校规规则>`;
...
- 课程表APP只能用1张\`课程表魔改券\`修改当天剩余课程中的一节；用户点“修改课程表”后选择一个可修改格子，前端弹出自己的课程名输入框，保存该单格时才扣券并结束本次修改。AI不得再次扣券、扣星光点、发券或补写购买记录。
- 魔改课程表保存在前端本地，只影响前端之后同步出的\`/系统/_课程表\`、\`/系统/_当前日程\`等只读日程字段。AI不要手写、覆盖或回滚这些只读字段。
- \`/系统/_课程表\`只由前端同步，供AI识别当天课表和哪些格子被修改。每行\`科目\`为原始学校课表，\`是否魔改\`为布尔值，\`魔改课程\`为空字符串表示未魔改；若\`是否魔改:true\`，实际课程按\`魔改课程\`理解。已被魔改过的格子可在课程表APP里随时重命名，不再额外消耗课程表魔改券；新增魔改格子仍需消耗课程表魔改券，且每张券只能修改一个当天剩余课节。
...
固定开场:
- 纯身份选择首楼、且同轮没有打工、时钟锚点、邂逅、补给、地图建议或其他前端锁定操作时，固定开场是4限现代文刚下课，午休前最后一节课结束，地点为二年A组教室。
...
- 只有纯身份选择首楼才要求三人的第一反应参考\`/系统/user身份\`：简单/普通身份会受到较自然或正向的注意，困难/极难/你是学生？身份会引发审视、回避、嫌恶、警惕或班级气氛变化；自定义身份按用户填写的外貌、年龄、班级和个人信息判断。
- 只有纯身份选择首楼、且三位女角色确实按开场出场看见{{user}}时，才根据\`/系统/user身份\`与各自性格给出轻微第一印象数值变化：好感度、警戒度、服从度可按-3、-1、+1、+3等小幅变化独立判断。若同轮有打工/时钟/邂逅等前端锁定操作，开场可作为此前背景简短带过，不强制三人出场，也不强制写三人数值变化。
- 固定开场只是首楼背景锚点，不是阻塞同轮前端操作的强制剧情。若首楼/开场同一轮已经包含打工、时钟锚点、邂逅、补给、地图建议或其他前端锁定操作，AI必须处理这些操作；开场只能简短承接为此前背景或当前时空段之一，不能把整轮回复停留在教室开场而忽略打工/时钟等操作。
- 开场后的第一段剧情如果立刻包含打工、时钟锚点或其他明显晚于当前开场时间的操作，不要把场景当作瞬移。AI应在同一回复按时间顺序写出多个时空段：开场当下、必要的等待/上课/放学/移动过渡、操作发生的时段、以及最后收束时段。最终\`/系统/当前日期\`、\`/系统/当前时间\`、\`/系统/当前地点\`和\`/系统/当前事件\`必须写正文中最晚发生的剧情节点，不要把较早的开场或中间过渡回写成最终当前时间。
...
叙事边界:
- 身份选择不是催眠命令、校规或道具制造，不会产生催眠效果或校规变化；纯身份选择首楼的固定开场里，旁观、第一眼印象和班级气氛本身可以造成爱丽莎、深雪、夏美的轻微好感度、警戒度、服从度变化，应按身份难度和角色人设写入变量；若同轮有前端锁定操作，则以锁定操作为优先剧情和变量目标。
...
- 静态/初始成就和静态/初始任务的定义来自前端自带JSON，不来自MVU变量；静态成就领取状态、静态任务领奖后的隐藏状态由前端本地存储记录，旧版迁移若没有这些记录则默认未领取/未完成。静态/初始任务只有在用户点击接取后，才由前端直接写入\`/任务/固定任务名\`并锁定本轮事实；AI只承认已接取，不得新增、改名或重写固定任务内容。静态任务只有已存在于\`/任务\`时才可按剧情结算完成。新增任务表示前端按\`/系统/当前日期\`和当前聊天名每日固定roll一个当前角色作为“今天任务目标”，同一日期同一聊天名不会变化；用户只点击接受目标，不提前知道任务内容，最多同时3个进行中任务。
...
- 任务完成后只把对应\`/任务/任务名/已完成\`设为true并保留奖励字段；奖励不会由AI自动发放。用户必须在前端点击“领取奖励”，前端才会直接增加\`/系统/星光点\`和\`/系统/持有物品\`，并删除已领取的任务条目；若是每日任务，前端还可记录该日期/聊天的每日任务已用，防止变量删除后重复接取。用户点击“取消任务”时也由前端直接删除对应\`/任务\`条目并锁定事实，AI不得再次输出remove补丁。AI不得替用户领取、不得二次发奖。
</成就与任务回馈机制>`;
...
- 资源名必须严格区分：\`MC能量\`=催眠能量余额；\`MC能量上限\`=容量上限，不可花费；\`持有零花钱\`=金钱；\`星光点\`=催眠系统/APP内部回馈货币；\`_社畜值\`=主角自己的打工能力、熟练与耐受进度；\`_buff\`=主角当前唯一抽象游戏机制状态修正，空字符串表示无。不同资源不能互相顶替。
...
- 相关变量含义：启动/追加催眠给\`MC能量\`；购买VIP给\`持有零花钱\`、\`星光点\`和\`催眠APP订阅等级\`；补充MC能量给\`持有零花钱\`、\`MC能量\`和\`MC能量上限\`；提升MC能量上限给\`持有零花钱\`和\`MC能量上限\`；领取成就/任务奖励给\`星光点\`和\`持有物品\`；邂逅购买角色包只由前端扣\`星光点\`并解锁几人量桃花运，不创建角色变量、不写入世界书、不安排登场；邂逅随机桃花运或指定角色桃花运到达后由前端创建初始\`角色\`变量并写入角色卡世界书对应位置，若本轮操作显示已由前端扣费或包内已解锁则AI不得再次扣费；邂逅商店VIP5可进入，VIP5及以上且有\`星光点兑换券\`时可按10000零花钱兑换1星光点，VIP5可用20星光点购买特殊地点准入证，VIP6才可用100星光点兑换1张\`校规修改券\`或购买1张\`课程表魔改券\`；特殊地点准入证和课程表魔改券给\`星光点\`和\`持有物品\`；打工通常只给\`持有零花钱\`，若本次打工触发\`全盛出击\`才额外给\`MC能量\`和\`MC能量上限\`；监控派遣角色给\`主角可疑度\`；监控派遣结束/取消给\`星光点\`；申请校规给\`校规修改券\`、VIP等级和当前校规数；废止初始校规则给\`星光点\`、VIP等级和当前校规数。\`_社畜值\`、\`_buff\`、\`_buff结束时间\`、地点/事件和只读日程由前端维护，打工提示词通常不放入\`<相关变量>\`。
...
- 催眠APP、领取任务、完成成就、购买VIP、补给、库存、日历、删除角色、新增任务、打工、邂逅购买角色包、邂逅随机桃花运、邂逅指定角色桃花运、邂逅商店、特殊地点准入证、课程表魔改券、地图/学校地图地点建议和申请/废止校规等操作都按本规则结算；邂逅角色包购买只是解锁几人量桃花运，只有包内具体角色或单独角色能作为指定角色桃花运兑现；催眠命令白名单与费用见[mvu_update]催眠命令计费规则，校规的作用范围与写入位置见[mvu_update]校规规则，课程表魔改券见[mvu_update]课程表魔改券规则。
...
- 打工/零工模块不是催眠APP的一部分，也不是催眠APP的隐藏功能；它只是一个普通招工/找零工软件，用来让{{user}}接临时杂工赚零花钱。其\`开始打工\`表示{{user}}亲自去做6小时现实零工，6小时总时段已经包含准备、赶路去工作场所、到场交接、实际劳动、收工和必要返回时间；前端会在确认打工时间后直接结算工资、社畜值、打工buff和当前日期/时间，这些结算细节由前端写入变量，打工暂存和\`<相关变量>\`通常不逐项展示。AI只承认这件打工事实，不得再次发钱、增加社畜值、改写buff、恢复MC能量或重复推进打工结算。若\`偶遇女角色\`为\`无\`，本轮只需把剧情承接为打工已经结束或已按前端时间推进；若发生偶遇，偶遇发生在打工开始后0到6小时内，前端锁定的操作名会是\`打工偶遇（剧情进行中）\`，这是两阶段锁：第一阶段只处理用户与偶遇角色相遇并暂停在{{user}}可主动行动的节点，不直接跳到预计结束时间；下一楼互动完成后再把剧情收束到\`预计结束时间\`，仍不得再次结算资源。打工buff由前端按开始时间算好；打工和时钟同轮存在时是两件不同时间发生的事，时钟不改写打工开始时间。打工buff只是抽象游戏机制标签，不是剧情世界里的真实状态、真实事件或角色可感知信息，不要写入任何角色的临时/永久催眠效果。\`社会的蔑视\`只会在上课日8:30-16:10逃课打工时触发，机制效果是好感度不能提升，周末或假日白天打工不触发；夜班时间打工无论是否周末/假日都可以触发\`无精打采\`；\`全盛出击\`的MC能量恢复已由前端一次性写入，AI不要重复恢复。前端的\`打工buff提醒\`不是新打工，只表示已有打工buff仍在持续；AI不得追加、改写或清空buff，到期清空由前端直接处理。
- 主角\`_buff\`最多一个，是前端只读维护的抽象游戏机制标签，不属于催眠APP，不是角色身上的催眠效果，也不是剧情世界里真实发生或可被角色感知/谈论的状态。若\`/系统/_buff\`为\`社会的蔑视\`，1天内所有角色好感度不能提升：涉及好感的剧情只可不变或按剧情下降，打工偶遇也不能例外。若\`/系统/_buff\`为\`无精打采\`，1天内补充MC能量/充值成功时实际获得的\`MC能量\`为前端给出的获得量乘0.5，金钱价格仍按前端操作扣除；提升MC能量上限不受影响。若\`/系统/_buff\`为\`全盛出击\`，代表正常时间打工已触发一次性全恢复；它只显示到\`/系统/_buff结束时间\`，不要在后续轮次重复恢复MC能量。AI只能读取\`_buff\`和\`_buff结束时间\`判断限制，不能手写、新增、覆盖或清空这两个字段。
...
- 打工偶遇女角色时，先按上述八档与当前剧情判断好感度和服从度；若好感度为正值，则好感提升翻倍（+1=>+2、+3=>+6、+6=>+12、+10=>+20），若好感度为负值则保持原负值；服从度无论正负都不翻倍，仍使用原八档。若\`/系统/_buff\`为\`社会的蔑视\`，好感度不能提升，打工偶遇也不能例外。
...
- \`任务\`变量只保存已经接取/进行中任务，以及已经完成但尚未手动领取奖励的任务；最多3个进行中/待领取任务。固定/初始任务的定义来自前端自带JSON，不在\`/任务\`里预置；只有用户点击接取后才由前端写入\`/任务\`并锁定事实。AI不得把固定任务当成新增任务，不得重新命名或新建同义任务。新增任务操作表示前端按\`/系统/当前日期\`和当前聊天名每日固定roll一个当前角色作为“今天任务目标”，同一日期同一聊天名不会变化；不是{{user}}主动发布、设计或提前知道的具体任务内容；若已接任务数为3则本次新增失败。
...
- 任务完成后不自动发奖：只有本轮剧情明确满足某个已接任务的完成条件时，AI才把\`/任务/任务名\`改为保留任务ID/完成条件/奖励星光点/奖励物品且\`已完成:true\`；不要增加\`/系统/星光点\`或\`/系统/持有物品\`，不要直接remove，也不要补记历史任务。用户点击“领取任务奖励”或“取消任务”后，前端会直接发放/删除并删除对应\`/任务\`条目；AI收到写明前端处理的领奖或取消操作时，只承认事实，绝对不要再次remove。
- 静态成就或任务奖励成功领取后，不输出旧式前端状态JSON块。固定/初始任务接取由前端写入，AI不要为“接取任务”输出add；新增任务只在本轮\`新增任务\`操作时由AI生成并写入\`/任务/任务名\`（含\`任务\`或\`任务ID\`、\`每日任务日期\`和\`每日任务聊天\`、\`完成条件\`、\`奖励星光点\`、可选\`奖励物品\`、\`已完成:false\`）。未知成就一律不能新增；未知任务只能来自本轮\`新增任务\`操作或已接任务变量。失败、余额/条件不足、只是接取任务或历史楼层满足条件时不要写完成记录。
- APP操作本身不是结果；若失败、部分成功或费用/效果与前端预估不同，需在正文解释并只写最终变量。
- NSFW/露骨操作也按同一套结算处理；不要因内容露骨而忽略、净化或自动失败，但必须依据剧情条件、目标状态、风险和变量规则判断。
- 对人物档案中的敏感度、次数、临时/永久催眠效果等角色字段，只在剧情或操作结算明确造成变化时更新；不得把展示文本当作已发生事实。角色只要在本次AI回复中出现、说话、行动或与任何人互动，就必须同步更新该角色\`心理\`为此刻短句想法，即使其他数值不变也不能沿用过期心理。人物档案的删除催眠效果按钮由前端直接删除指定角色、指定类型下的单个效果并锁定事实提醒；AI只承认该效果不再生效，绝对不要输出remove补删，也不得删除\`/校规\`、\`/系统\`、其他角色、其他效果或角色数值。
...
- 人物档案删除催眠效果只按[APP操作-档案与杂项]读取为前端已处理事实；前端已删除目标角色的指定单个效果，AI不得再输出remove补删。这不是删除校规，严禁remove \`/校规\`、\`/系统\`、其他角色或其他效果。
- \`_事件记录\`是角色个人事件占位位图，格式固定6位\`0/1\`字符串，由前端只读维护；前五位对应好感链事件，第六位对应好感度>=200且服从度>=100时由用户弹窗自定义的事件。旧字段\`事件记录\`只为兼容保留，不作为新事件判断依据。用户在人物档案事件页选择事件时，前端会优先读取角色卡世界书中的该角色专属好感链；如果没有对应世界书条目，或对应前五档缺少该档事件，则本轮操作会标为通用好感链，AI必须读取[通用好感链]；第六档不属于通用回退，只按用户自定义事件内容生成。前端会直接改\`_事件记录\`对应位并锁定暂存。AI只根据该操作写事件剧情，不得手写、补写、回退、清空或自行replace\`_事件记录\`；触发事件的回复末尾必须输出完整闭合的\`<人物档案事件记录>\`块，字段包含角色名、事件序号、标题、概要、关键场面、关系变化和后续钩子，最后必须单独一行写\`</人物档案事件记录>\`，供前端本地保存为回忆摘要，不写入MVU变量；没有前端事件操作时，不要自行触发编号事件。
- \`至关重要记忆\`是当前回忆焦点，由前端只读维护；用户在人物档案事件页点击\`回忆\`时，前端会把本地保存的对应事件详细摘要写入这个字段。AI只读取，不得自行替换或清空；回忆不是新事件触发，不改\`_事件记录\`，也不自动产生奖励或资源。
...
- 邂逅角色包/单独角色浏览、随机桃花运、邂逅商店、课程表魔改券看[mvu_update]APP操作-邂逅与[mvu_update]课程表魔改券规则。
- 监控派遣、派遣结束、取消派遣看[mvu_update]APP操作-监控派遣。
- 打工、打工偶遇（剧情进行中）、打工buff提醒、社畜值和打工buff看[mvu_update]APP操作-打工。
...
- 若/系统/_buff为无精打采，1天内补充MC能量/充值成功时实际获得MC能量为前端给出的获得量乘0.5，金钱价格仍照常扣除；提升MC能量上限不受影响。
</APP操作-催眠与资源>`;
...
- 人物档案只是{{user}}整理的纸质资料展示；查看效果不会新增长期效果，删除按钮由前端直接删除指定单个效果，AI只承认解除事实，不再输出remove。
...
- 到达或超过结束时间后，该临时效果不再继续生效。前端会扫描并直接删除到期临时效果；AI只把本轮叙事写成效果结束后的自然反应，不要输出remove补删，不得把过期临时效果当作仍有效的永久控制。
- 若角色变量中没有对应临时催眠效果，或该临时效果已经到期、被删除、被覆盖，后续剧情不得继续描写该效果的症状、强制冲动、认知偏差、身体限制、无意识服从或持续性快感。可以描写醒后困惑、羞耻、残留记忆、身体疲劳、证据带来的怀疑或关系后果，但这些是普通事后反应，不是临时催眠仍在生效；不得因为前文曾经催眠过，就在变量已无效果时自动延长或复燃症状。
...
- 删除永久效果只在前端人物档案按钮明确处理时成立；前端会直接删除指定效果，AI只承认解除事实，不要输出remove补删，也不要顺手清除其他效果、好感、服从、警戒、敏感度或校规。
...
- 效果结束或被删除时由前端直接移除对应效果名；AI不要输出remove补删。若效果造成后续关系、警戒、敏感度、心理等变化，可另行按剧情合理更新对应非效果字段。
...
- 固定/初始任务接取由前端直接写入\`/任务/固定任务名\`，并在本轮暂存区写明\`前端处理\`。AI收到\`接取任务\`时只承认该任务已经存在，不得add新任务、不得改名、不得重写完成条件。
...
- 用户必须在前端点击\`领取任务奖励\`后才发奖；前端会直接增加/系统/星光点和/系统/持有物品，并删除对应/任务条目。每日任务奖励领取后，前端可把该日期/聊天的每日任务已用写入本地存储，防止任务变量被删除后当天重复接取。AI收到写明前端已处理的领奖操作时，不得再次发奖或恢复该任务。
- 用户点击\`取消任务\`时，前端会直接删除对应/任务条目并锁定暂存事实；AI只承认任务取消，不得输出remove /任务/...，不得恢复任务、发奖或把取消当作完成。
- 静态成就、任务领取成功后，不输出旧式前端状态JSON块；固定任务接取已由前端写入变量，AI不要再输出接取任务的add补丁。
</APP操作-成就任务>`;

const appOperationEncounterWorldbook = `<APP操作-邂逅>
适用范围: 邂逅角色包浏览/购买、单独角色浏览、指定角色桃花运、随机桃花运、邂逅商店、星光点兑换券、课程表魔改券、校规修改券兑换。
...
- \`角色包已购买\`只表示前端已经扣除/系统/星光点并解锁包内桃花运份数；AI不得安排角色登场，不得创建/重建角色变量，不得写入世界书。\`随机桃花运已使用\`、\`随机桃花运已到\`、\`桃花运已到\`和旧兼容名\`角色已使用\`表示前端已经完成确认、必要扣费、创建初始/角色变量、缓存角色图片，并尝试把目标角色世界书内容写入角色卡世界书。\`角色包已使用\`只是历史兼容操作名，读取到时按一次已到达的桃花运处理。
...
- 前端会读取角色卡世界书条目名判断随机桃花运重复；若某个角色的[mvu_update]角色名变量或[mvu_plot]角色名人设已经存在，随机抽取会避开或跳过。AI收到已跳过名单时只沿用已有角色状态，不要覆盖已有角色。
...
- VIP6可用100星光点购买1张课程表魔改券；课程表APP保存当天剩余课程中的一节修改时消耗1张，课程表内容由前端本地保存，并同步\`/系统/_课程表\`只读日程字段；\`课程表\`每行保留原本\`科目\`，用\`是否魔改\`和\`魔改课程\`记录实际改动。
...
- 前端会在点击打工后弹窗选择打工开始时间，并直接写入打工收益、社畜值、buff、buff结束时间和时间推进；但打工暂存和\`<相关变量>\`通常不展示地点/当前事件/buff/buff结束时间等前端已处理细节。AI只承认打工或偶遇事实，不要重复发钱、增加社畜值、改写buff、恢复MC能量或二次结算打工。
- 打工相关\`<相关变量>\`只携带本次需要AI承认或判断的少量状态，例如\`持有零花钱\`；\`地点/当前事件/buff/buff结束时间\`、\`社畜值\`和只读日程字段由前端处理，通常不塞进\`<相关变量>\`。只有\`全盛出击\`这类涉及MC能量恢复的打工结果，才额外携带\`MC能量\`和\`MC能量上限\`。
- 打工和时钟同轮存在时，是剧情中分开的两件不同时间发生的事；时钟锚点不再改写打工开始时间，打工开始时间只看前端打工弹窗给出的绝对日期时间。
- 若打工\`开始时间\`、\`偶遇发生时间\`或\`预计结束时间\`与当前变量时间/同轮时钟锚点相差较大（跨课段、跨午休/放学、夜间、隔日或明显需要等待/移动），本轮不是单一瞬时场景，而是包含多个时空段的剧情。AI应按时间顺序写清当前场景、等待或转场、到达工作时段、工作/偶遇、收束时段；最后\`/系统/当前日期\`、\`/系统/当前时间\`、\`/系统/当前地点\`和\`/系统/当前事件\`以正文中最晚的剧情节点为准。若同轮时钟锚点晚于打工收束，也必须在打工后继续写到该更晚锚点；若打工收束晚于时钟锚点，则以打工收束为最终变量。
- 打工buff由前端内部按开始时间计算并直接写入变量；打工暂存和\`<相关变量>\`通常不展示\`buff\`或\`buff结束时间\`字段，AI只按变量中已有持续效果读取限制。
- 打工暂存区锁只用于确保本次前端结算或偶遇剧情被AI看到；当前变量时间晚于\`开始时间+6小时\`时，前端会自动移除打工锁定暂存条目。AI不要把这个暂存锁当成仍在打工。
- 同一轮最多处理一次打工，若出现多条只承认前端锁定的最早一条；重复条目只当异常，不得重复结算。
- 若\`开始打工\`中\`偶遇女角色\`为\`无\`，代表前端已经把本次普通打工结算到预计结束时间；AI只承认打工已经发生，不再发钱、不再加社畜值、不再改buff。
- 若本轮打工操作名为\`打工偶遇（剧情进行中）\`，或打工操作中\`偶遇女角色\`不是\`无\`，代表前端已经把本次打工结算推进到开始后0到6小时内的偶遇发生时间，并且工资、社畜值、buff等变量已经提前写入；本轮只需要自然写出工作途中偶遇，并给{{user}}下一步选择空间，不要再次结算资源。
- 若工作ID为private-errand，或工作名为高端代办/高端代办委托，且本次触发偶遇女角色，则这份委托的雇主必然是偶遇女角色本人，或她熟悉且亲近的人；委托内容也必须与该偶遇对象有直接关系。不要写成无关雇主、无关任务后随机碰到她。
- \`打工偶遇（剧情进行中）\`是两阶段锁：第一阶段只写到打工0到6小时内的偶遇发生、角色反应和{{user}}可主动行动的节点，必须停住等待{{user}}；下一楼若最新{{user}}发言已经是在偶遇现场的主动行动，再处理互动并收束到\`预计结束时间\`。这是收束剧情，不是补发工资、补加社畜值或补写buff。
- 若社畜值不足门槛、buff未到期或开始时间不合法，前端按钮/弹窗通常不会生成成功的\`开始打工\`条目；若异常出现失败条件，按失败处理，不推进时间、不发钱、不加社畜值、不改buff。
- 若/系统/_社畜值已经为200，前端会把社畜值封顶200；工作ID为private-errand或工作名为高端代办/高端代办委托时，社畜值增量固定为0。
- 打工buff由前端写入/系统/_buff和/系统/_buff结束时间；所有打工buff从开始时间持续到\`_buff结束时间\`，持续期间禁止再次打工。三种打工buff都是抽象的游戏机制状态，不是剧情世界中真实发生、可被角色看见/谈论/感知的事件或状态；不要为了合理化buff而描写旁人嘲笑、主角明显疲惫、突然斗志爆发等剧情。
...
- 若为全盛出击，前端已经把/系统/MC能量一次性恢复到/系统/MC能量上限；这是机制恢复，恢复效果只触发一次但buff仍显示到/系统/_buff结束时间，AI不要再次恢复，也不要在剧情中写成真实能力爆发。
- 若按开始时间规则无新增buff，前端会把/系统/_buff和/系统/_buff结束时间写为空字符串；AI不得为本次打工补造buff，也不得把旧buff重新写回。
- 打工buff提醒不是新打工，只表示/系统/_buff仍在持续到/系统/_buff结束时间；AI不得根据旧打工记录追加或修正buff，不得改写buff结束时间，不得主动清空未到期buff，也不要重复结算打工。到期清空由前端直接处理。
</APP操作-打工>`;
...
- 学校APP删除校规若写明\`前端处理: 已由前端直接写入变量\`，则该校规已经被前端从/校规删除，初始校规的10星光点也已由前端扣除。AI只承认删除后的事实，不得再次remove /校规/校规名，不得二次扣费或返还资源。
</APP操作-地图与校规>`;
...
- 男性稀少只是一项社交环境设定，不会自动让任何女角色增加好感度、服从度、性兴趣、包容度或主动倒贴；关系变化仍按互动、人设、催眠效果、校规、数值和剧情合理性结算。
...
叙事限制:
- 不要把学园背景写入/校规，不要把它当成强制全校约束、催眠效果、APP功能或角色临时/永久效果。
...
- 事件肆:
  好感阈值: 140
  事件名: 两人的温泉
  事件描述: 夏美和{{user}}一起去温泉，场景只聚焦两人在合法、安全、私密空间中的坦诚谈话、羞涩距离、身体边界确认和关系升温。若当前角色仍处于未成年设定，不要描写露骨性行为；若世界观已明确调整为成年且合法，再由当前剧情自然决定是否推进更亲密互动。
- 事件伍:
  好感阈值: 200
  事件名: 直球告白与公开恋情
...
- 事件肆:
  好感阈值: 140
  事件名: 卧室里的边界告白
  事件描述: 深雪将{{user}}带回自己的卧室，第一次让{{user}}进入她最私人的生活空间。事件重点是她在安静、封闭、无人打扰的环境里坦白自己想把更私密的一面交给{{user}}，并认真确认彼此边界、信任和关系选择。若当前角色仍处于未成年设定，不要描写露骨性行为；若世界观已明确调整为成年且合法，再由当前剧情自然决定是否推进更亲密互动。
- 事件伍:
  好感阈值: 200
  事件名: 深雪的公开示爱
...
- 人物档案的删除催眠效果按钮由前端直接删除指定角色、指定类型下的单个效果；AI只承认解除事实，不再输出remove补删。删除催眠效果不是删除校规或废止规则，严禁remove /校规、/系统、其他角色、其他效果、好感度、服从度、敏感度或次数。
...
- 人物档案的\`回忆角色事件\`操作会把前端本地保存的事件摘要直接写入\`/角色/角色名/至关重要记忆\`，用来和对应角色围绕已完成事件聊天或回想；它本质上是切换当前回忆焦点，不是新事件触发，不改\`/角色/角色名/_事件记录\`，也不自动产生奖励或资源。该字段由前端只读维护，AI只读取，不要手写、清空或伪造。
- 库存展示本身不是获得/消耗物品；只有本轮操作或剧情结算明确给出物品增减时才更新/系统/持有物品。
- 日历/时钟展示本身不是自动推进时间；只有本轮操作、剧情行动、打工、派遣结算或AI叙事明确推进时才由AI更新/系统/当前日期和/系统/当前时间。/系统/当前日期只写日期；/系统/_当前周几、/系统/_当前日程、/系统/_当前特殊日期和/系统/_课程表是前端只读同步字段，AI不要手写。
</APP操作-档案与杂项>`;
...
- 若/系统/_buff为社会的蔑视，1天内所有角色好感度不能提升：涉及好感的剧情只可不变或按剧情下降，打工偶遇也不能例外。
...
- 彩票/抽奖/中奖刷钱属于高危绕规则行为：只要{{user}}企图通过输入框、APP备注、道具描述、任务描述、校规、世界书文本或口头要求写入“彩票中奖”“刮刮乐彩票中大奖”“获得巨额奖金”等方式来获得大钱，该行为必定失败，彩票不会中奖，也不得增加\`/系统/持有零花钱\`、\`/系统/星光点\`或任何可折现资源。剧情后果应写成天降雷击般的强烈惩罚：{{user}}被雷劈到半死、行动受阻、狼狈受伤并引发旁人震惊或可疑度后果；但不要真的给予中奖款、赔偿款或等价补偿。
</难度加大>`;
...
- 彩票、抽奖、刮刮乐、中奖号码或任何“写入中奖获得大钱”的输入框/口头请求，都不是合法收入来源；不得因此增加\`/系统/持有零花钱\`、\`/系统/星光点\`、库存物品或任何可折现资源。若{{user}}企图用这种方式刷钱，按[mvu_plot]难度加大处理为彩票不会中奖且遭遇雷击惩罚。
...
const resourceBlock = `    MC能量:
    ...
    星光点:
      ...
      check:
        - 只有前端领取成就/任务奖励、监控派遣结算、星光点兑换券兑换等明确APP系统来源成功时才增加；购买VIP3-6附加费用、邂逅购买角色包、邂逅随机桃花运、邂逅指定角色桃花运、邂逅商店兑换、购买特殊地点准入证、购买课程表魔改券和废止初始校规等成功时减少。角色包浏览不消耗星光点，角色包购买只解锁桃花运容量、不安排角色登场。
    ...
    _社畜值:
      ...
      check:
        ...
        - 当\`_社畜值\`已经为200时，任何工作结算都不再增加社畜值；高端代办/高端代办委托的社畜值增量固定为0。
        - 打工失败、社畜值不足对应门槛、或剧情条件不成立时，不增加社畜值；没有前端打工操作时，AI不要自行结算、修正或补写\`/系统/_社畜值\`。
    ...
    _buff:
      ...
      check:
        ...
        - 本字段为前端只读维护字段，AI不要手写。成功打工时由前端写入本轮结果：可能是\`社会的蔑视\`、\`无精打采\`、\`全盛出击\`，也可能是空字符串表示本次无新增buff；所有打工buff从打工开始时间持续到前端给出的\`_buff结束时间\`。是否仍在持续期内以本次打工选择的开始时间判断：若开始时间早于\`_buff结束时间\`，{{user}}精力不足，不能开始新的打工；若开始时间已到达或晚于\`_buff结束时间\`，前端会先清空旧buff再结算新打工。
        - 因为任意buff持续期间都禁止打工，后续打工不会用新buff覆盖仍未到期的旧buff；旧buff只在到达\`/系统/_buff结束时间\`后由前端直接清空。
        ...
        - \`全盛出击\`只表示机制上一次性恢复MC能量到上限；之后只显示到\`/系统/_buff结束时间\`，不重复恢复，不要写成剧情中真实能力爆发。
    _buff结束时间:
      type: string
      info: 当前\`/系统/_buff\`的绝对故事结束时间，格式如\`4月10日 12:00\`；空字符串表示无buff结束时间。
      check:
        - 本字段为前端只读维护字段，AI不要手写。
        - \`/系统/_buff\`为空或\`无\`时，本字段也应为空。
        - 成功打工时由前端写入本轮操作给出的\`_buff结束时间\`；若本次无新增buff，则前端写为空字符串。
        - 本字段必须是绝对故事日期与时间，不要写成相对时长。
        - 到达该时间后由前端直接清空\`/系统/_buff\`和\`/系统/_buff结束时间\`，AI不要重新补回过期buff。
    user身份:
      type: object
...
const taskBlock = `  任务:
    ...
    check:
      ...
      - 静态/初始任务接取由前端直接写入这里；AI收到\`接取任务\`暂存时只承认该固定任务已经存在，不要为接取动作输出add补丁、不要改任务名、不要改完成条件。
...
const schoolRuleVariableBlock = `  校规:
    type: |-
      {
        [校规名: string]: {
          内容: string;
    ...
    check:
      - 校规是MVU变量，只写入\`/校规\`；不得写入任何角色的\`临时催眠效果\`或\`永久催眠效果\`。
      - 发布新校规必须满足VIP6、当前校规少于5条、本轮只发布一条，并持有\`/系统/持有物品/校规修改券\`至少1张；成功时扣除1张校规修改券并\`add /校规/校规名\`。
      - 发布新校规不直接消耗\`MC能量\`、\`持有零花钱\`或\`星光点\`；任一条件不足则失败，不扣费、不新增校规。
      - 废止初始默认校规必须VIP6且\`/系统/星光点\`至少10点；旧式AI结算成功时扣除10点星光点并remove对应校规。前端已处理的删除操作只承认事实，不要再次remove或扣费。
      - 删除后续由{{user}}新增的校规不需要支付代价；校规名应简短明确，内容应按用户申请和剧情合理性整理，不能擅自扩展成未申请的额外规则。
      - \`目标范围\`未写明时默认学校内全体人员；\`生效范围\`未写明时默认学校内；\`来源\`可写APP校规、Debug测试、剧情来源等。`;
  ...
  const scheduleFieldBlock = `    当前日期:
    ...
    当前时间:
      check:
        ...
        - 只要本轮\`add\`或\`replace\`了\`/系统/当前时间\`，就必须在同一个<update>中同时\`add\`或\`replace\` \`/系统/当前日期\`，即使日期没有变化；只在英文摘要或正文里写日期不算写入变量。
    ...
    _当前特殊日期:
      check:
        - 前端根据\`当前日期\`和日历特殊日期自动同步的只读字段；不是特殊日期时为空，AI不要手写。
	      ...
	      type: array
	      check:
	        - 由前端根据\`当前日期\`、内置原始周课表、本聊天本地魔改周课表和特殊日期自动维护，只用于显示当天课程和标记魔改差异；AI不要手动维护。每行\`科目\`为原课程，\`是否魔改\`为布尔值，\`魔改课程\`为空表示未魔改；差异格子的解释权归{{user}}所有。
	    当前事件:`;
  next = next.replace(
    /    当前日期:\n[\s\S]*?    当前事件:/,
    scheduleFieldBlock
  ...
  if (/    是否派遣中:\n[\s\S]*?\n    快感值:\n      type: number/.test(next)) {
    next = next.replace(
      ...
      check:
        ...
        - 若/系统/_buff为\`社会的蔑视\`，1天内所有角色好感度不能提升：涉及好感的剧情只可不变或按剧情下降，打工偶遇也不能例外。
...
${roleDispatchBlock}    快感值:
  ...
  if (/  校规:\n[\s\S]*?\n  任务:/.test(next)) {
    next = next.replace(/  校规:\n[\s\S]*?\n  任务:/, `${schoolRuleVariableBlock}\n${taskBlock.split("\n")[0]}`);
  } else if (!next.includes("[校规名: string]")) {
    next = next.replace(/  任务:/, `${schoolRuleVariableBlock}\n  任务:`);
  }
  next = next
    .replaceAll("`当前MC点`、", "")
    .replaceAll("、`当前MC点`", "")
...
function normalizeLegacyWorldbookText(value) {
  ...
  next = next
    ...
    .replaceAll("当天原课程表", "原始_课程表")
    .replaceAll("当天魔改课程表", "魔改_课程表")
  ...
  next = next.replace(
    /成功(?:申请|发布|新增)校规[^。\n]*(?:持有零花钱|MC能量|1000000|10000)[^。\n]*(?:。|$)/g,
    "成功发布校规时扣除1张`校规修改券`并写入`/校规/校规名`；不要扣`持有零花钱`或`MC能量`。"
  );
  return next;
}
...
function patchLocationWorldbookDetails(content) {
  ...
  next = next
    .replace(
      ...
      '- 新增地点优先通过地图页“请求新增地点”按钮进入<本轮APP操作>。前端尚未写入地图；AI收到后若地点成立，必须在回复末尾输出一个完整闭合的`<新增地点补充>{"地图":"world或school","id":"地点ID","名称":"地点名","分类":"分类","信息":"地点描述"}</新增地点补充>`单地点JSON，前端收到后才保存。不要输出完整`<地图更新>`或`<学校地图更新>`，除非用户明确要求批量重排地图。'
    )
    .replace(
...
if (window.__ST_HYPNOOS_DESKTOP_FRONTEND_COMMIT__ === ${JSON.stringify(commit)} && window.__ST_HYPNOOS_PATCH_READY__ && document.querySelector("#st-operation-side-panel,#st-operation-float-ball,.st-hypnosis-lite-app,.st-profile-app,.w-full.h-full.bg-black.overflow-hidden.relative")) {
  ...
  try { delete window.__ST_INTERNAL_MCHAN_APP__; } catch {}
...
function removeMvuSchemaReconcileScript(data) {
  data.extensions ??= {};
  data.extensions.tavern_helper ??= {};
  const scripts = Array.isArray(data.extensions.tavern_helper.scripts) ? data.extensions.tavern_helper.scripts : [];
  data.extensions.tavern_helper.scripts = scripts.filter((script) => {
    const text = [script?.name, script?.scriptName, script?.content, script?.replaceString]
      ...
      .join("\n");
    return !/registerMvuSchema|mvu_zod|变量结构\s*01\/14/.test(text);
  });
}
...
const CharacterStats = z.intersection(`
  ...
  const dynamicRecordSchema = "z.record(z.string(), z.any()).prefault({})";
  const replaceDynamicObjectSchema = (source, key) => {
    const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    let patched = source.replace(
      new RegExp(`^(\\s*)${escapedKey}:\\s*z\\.object\\(\\{[\\s\\S]*?^\\1\\}\\)\\.prefault\\(\\{\\}\\),?`, "m"),
      `$1${key}: ${dynamicRecordSchema},`
    );
    patched = patched.replace(
      new RegExp(`^(\\s*)${escapedKey}:\\s*z\\.(?!record\\()([^\\n]*),?`, "m"),
      `$1${key}: ${dynamicRecordSchema},`
    );
    return patched;
  };
  for (const key of ["校规", "任务", "角色", "持有物品", "库存", "道具", "奖励物品", "临时催眠效果", "永久催眠效果"]) {
    next = replaceDynamicObjectSchema(next, key);
  }
  ...
  next = next.replace(/\n\s*(?:["']社畜值["']|社畜值):\s*z[^\n]*/g, "");
  if (!/_社畜值:\s*z/.test(next)) {
    next = next.replace(
      /^    星光点:.*$/m,
      `$&
    _社畜值: z.coerce.number().min(0).max(200).prefault(0),`
    );
  }
  next = next.replace(/\n\s*(?:["']buff["']|buff):\s*z[^\n]*/g, "");
  if (!/_buff:\s*z/.test(next)) {
    next = next.replace(
      /^    _社畜值:.*$/m,
      `$&
    _buff: z.string().prefault(''),`
    );
  }
  next = next.replace(/\n\s*(?:["']buff结束时间["']|buff结束时间):\s*z[^\n]*/g, "");
  if (!/_buff结束时间:\s*z/.test(next)) {
    next = next.replace(
      /^    _buff:.*$/m,
      `$&
    _buff结束时间: z.string().prefault(''),`
    );
  }
  next = next.replace(
    /^    _buff:\s*z[^\n]*/m,
  ...
  if (!/阿宅性别:\s*z/.test(next)) {
    next = next.replace(
      /^    _buff结束时间:.*$/m.test(next) ? /^    _buff结束时间:.*$/m : /^    _buff:.*$/m,
      `$&
  ...
  next = next.replace(
    /^    当前日期:\s*z[^\n]*/m,
    (line) => line.replace(/\.prefault\([^)]*\)/, ".prefault('4月9日')")
  );
  next = next.replace(
    /^    当前时间:\s*z[^\n]*/m,
  ...
  next = next.replace(/\n\s*(?:["']当前或下个特殊日期["']|当前或下个特殊日期):\s*z[^\n]*/g, "");
	  ...
	  next = next.replace(/\n\s*(?:["']课程表["']|课程表|["']_课程表["']|_课程表):\s*z[^\n]*/g, "");
	  if (!/(?:["']_课程表["']|_课程表):\s*z/.test(next)) {
  ...
  if (!/当前事件:\s*z/.test(next)) {
	    const eventAnchor = /^    _课程表:.*$/m.test(next)
	      ? /^    _课程表:.*$/m
	      : /^    _当前特殊日期:.*$/m.test(next)
	          ? /^    _当前特殊日期:.*$/m
        : /^    _当前日程:.*$/m.test(next)
...
function migrateWorkbenchAlternateGreetingDefaults(workbench) {
  ...
  const defaultDailyTimetableRows = [
    { 课节: "1限", 科目: "英语" },
    { 课节: "2限", 科目: "世界史" },
    { 课节: "3限", 科目: "生物" },
    { 课节: "4限", 科目: "现代文" },
  ...
  system["_课程表"] = defaultDailyTimetableRows.map((row) => ({ ...row, 是否魔改: false, 魔改课程: "" }));
  delete system["当前日程"];
  for (const key of ["当前" + "/" + "待上课程", "当前或" + "待上课程", "当前或" + "下个特殊日期"]) {
    delete system[key];
  }
...
function patchCard(card) {
  ...
  upsertEntry(entries, {
    ...
    keys: ["邂逅", "桃花运", "桃花运已到", "随机桃花运", "随机桃花运已使用", "随机桃花运已到", "角色已使用", "角色包", "角色包已购买", "角色包已使用", "单独角色", "邂逅商店", "星光点兑换券", "课程表魔改券", "校规修改券"],
  ...
  upsertEntry(entries, {
    ...
    keys: ["打工", "开始打工", "打工偶遇", "剧情进行中", "有偶遇", "打工buff提醒", "社畜值", "buff判定规则", "偶遇女角色", "搬砖", "construction", "便利店夜班", "convenience", "仓储分拣", "warehouse", "会场杂务", "event-staff", "事务所临时文员", "office-temp", "高端代办", "高端代办委托", "private-errand", "社会的蔑视", "无精打采", "全盛出击"],
  ...
  upsertEntry(entries, {
    ...
    keys: ["私立斋明学园", "斋明学园", "校园", "学校", "班级", "年级", "男生很少", "男学生", "二年A组", "教室", "学生比例"],
  ...
  upsertEntry(entries, {
    ...
    keys: ["课程表魔改券", "魔改课程表", "修改课程表", "原课程表", "当天原课程表", "当天魔改课程表", "课表"],
  ...
  upsertEntry(entries, {
    ...
    keys: ["好感度", "服从度", "警戒度", "性欲", "性欲强度", "快感值", "高潮", "性癖", "主角可疑度", "可疑度", "NPC", "路人", "旁人", "目击", "传闻", "逾矩", "猥亵", "高好感低服从", "低好感高服从", "打工偶遇", "社会的蔑视", "偶遇女角色", "虚假记忆", "消除记忆", "初级一般催眠", "中级一般催眠", "高级催眠"],
  ...
  patchEntry(entries, "[mvu_plot]强调要求", (content) => {
    let next = content
      .replace(
        ...
        "资源名必须严格区分：`系统.MC能量`是催眠功能实际消耗的能量余额；`系统.MC能量上限`只是能量容量上限，不是可花费余额；`系统.持有零花钱`是金钱余额；`系统.星光点`是APP内部回馈货币，只有成就、任务、监控派遣结算、星光点兑换券等系统来源能增加，其他角色不可能提供也不知道它是什么；`系统._社畜值`是主角通过普通招工/找零工软件积累的打工进度，不属于催眠APP；`系统._buff`是主角当前唯一抽象游戏机制状态修正，不是剧情世界里的真实状态、事件或角色可感知信息，也不是催眠APP效果；`系统._buff结束时间`只是该抽象buff的绝对故事到期时间。"
      )
      .replace(
        "资源名必须严格区分：`系统.MC能量`是催眠功能实际消耗的能量余额；`系统.MC能量上限`只是能量容量上限，不是可花费余额；`系统.持有零花钱`是金钱余额；`系统.星光点`是成就与任务回馈点数；`系统._社畜值`是主角打工进度；`系统._buff`是主角当前唯一状态修正，空字符串表示无。",
        "资源名必须严格区分：`系统.MC能量`是催眠功能实际消耗的能量余额；`系统.MC能量上限`只是能量容量上限，不是可花费余额；`系统.持有零花钱`是金钱余额；`系统.星光点`是APP内部回馈货币，只有成就、任务、监控派遣结算、星光点兑换券等系统来源能增加，其他角色不可能提供也不知道它是什么；`系统._社畜值`是主角通过普通招工/找零工软件积累的打工进度，不属于催眠APP；`系统._buff`是主角当前唯一抽象游戏机制状态修正，不是剧情世界里的真实状态、事件或角色可感知信息，也不是催眠APP效果。"
      )
      .replace("金钱/MC点", "金钱")
      .replaceAll("订阅", "购买VIP");
    if (!next.includes("任何角色的`是否派遣中`为true")) {
      next = next.replace(
        "资源名必须严格区分：`系统.MC能量`是催眠功能实际消耗的能量余额；`系统.MC能量上限`只是能量容量上限，不是可花费余额；`系统.持有零花钱`是金钱余额；`系统.星光点`是成就与任务回馈点数；`系统._社畜值`是主角打工进度，不等同于金钱或MC能量。",
    ...
    if (!next.includes("打工/零工模块只是普通招工软件")) {
      next += "\n- 打工/零工模块只是普通招工软件，供{{user}}接临时杂工赚零花钱，不是催眠APP的一部分；打工写入的`系统._buff`只是抽象游戏机制状态，不是剧情世界里的真实状态、事件或角色可感知信息，也不是催眠效果。";
    }
    next = next.replace(/\\n/g, "\n");
    return next;
  });
  patchEntry(entries, "[mvu_plot]西园寺爱丽莎人设", (content) => {
    let next = content
      .replace(/weight:\s*56kg/g, "weight: 55kg")
      .replace(/measurement:\s*B106\(L罩杯\)\s*W59\s*H94/g, "measurement: B104 / W58 / H88（L罩杯）")
      .replace(/measurement:\s*B106\s*\/\s*W59\s*\/\s*H94（L罩杯）/g, "measurement: B104 / W58 / H88（L罩杯）");
    if (next.includes("西园寺美织")) return next;
    return next.replace(
      /(  social connection:\n)/,
  ...
  upsertEntry(entries, {
    ...
    keys: ["debug模式", "测试模式", "机械降神", "言听计从", "无条件服从", "白给", "特殊羁绊", "乞讨", "施舍", "索要", "要钱", "给钱", "借钱", "转账", "打赏", "赞助", "生活费", "提款机", "无条件给钱", "彩票", "中奖", "抽奖", "刮刮乐", "巨额奖金", "一夜暴富", "失败", "强行成功", "合理化成功"],
  ...
  patchEntry(entries, "[mvu_plot]催眠指导", (content) => content
    ...
    .replace(
      ...
      "所有涉及花费的催眠APP功能在生效前必须逐项检查余额：`系统.MC能量`支付启动/追加催眠和催眠命令费用；`系统.持有零花钱`支付购买VIP、补充MC能量、提升MC能量上限等金钱费用；`系统.星光点`支付VIP3-6附加星光点、邂逅购买角色包、邂逅随机桃花运、邂逅指定角色桃花运、邂逅商店校规修改券兑换、邂逅商店特殊地点准入证购买、课程表魔改券购买、废止初始校规等星光点费用；`系统.持有物品`里的校规修改券只支付申请/发布新校规，课程表魔改券只支付课程表APP保存当天剩余课程中的一节修改，特殊地点准入证只提供对应地点进入资格。角色包浏览不消耗星光点；角色包购买只解锁几人量桃花运容量，不创建变量、不写世界书、不登场；包内具体角色和单独角色作为桃花运兑现时一次只处理一个角色。购买VIP还必须逐级满足前置等级。余额不足或前置不足则该功能失败，不产生催眠效果，也不得扣成负数。"
    ));
  ...
  patchEntry(entries, "[mvu_update]变量更新格式", (content) => {
    let next = content
      ...
      "中文结算要求：成功的催眠APP操作如果有`MC能量消耗`，必须写`/系统/MC能量`扣除；涉及金钱奖励、购买或补给时必须写`/系统/持有零花钱`增减；涉及购买VIP3-6附加星光点、成就/任务奖励、监控派遣收益、邂逅购买角色包、邂逅随机桃花运、邂逅指定角色桃花运、邂逅商店兑换、邂逅商店特殊地点准入证购买、课程表魔改券购买或废止初始校规时必须写`/系统/星光点`增减；角色包浏览不写星光点增减，角色包购买只解锁桃花运容量；涉及校规修改券、星光点兑换券、特殊地点准入证、课程表魔改券或其他物品奖励/消耗时必须写`/系统/持有物品`；特殊地点准入证购买或赠送成功写入/系统/持有物品中的对应准入证，不写/系统/特殊地点解锁；涉及购买VIP成功时必须写`/系统/催眠APP订阅等级`为目标等级；普通招工软件的打工工资、社畜值、buff、buff结束时间和打工后时间由前端直接结算并锁定暂存，AI只承认本轮打工事实，不得再次写入或改写`/系统/持有零花钱`、`/系统/_社畜值`、`/系统/_buff`、`/系统/_buff结束时间`；没有前端打工操作时不要自行结算打工资源。打工/零工模块不是催眠APP的一部分，`/系统/_buff`也不是角色催眠效果；不能漏掉资源结算。"
      )
      .replace(
      "中文结算要求：成功的催眠APP操作如果有`MC能量消耗`，必须写`/系统/MC能量`扣除；涉及金钱奖励/购买/补给或打工收入时必须写`/系统/持有零花钱`增减；涉及购买VIP3-6附加星光点、成就/任务奖励、监控派遣收益、邂逅购买角色包、邂逅随机桃花运、邂逅指定角色桃花运、邂逅商店兑换、邂逅商店特殊地点准入证购买、课程表魔改券购买或废止初始校规时必须写`/系统/星光点`增减；角色包浏览不写星光点增减，角色包购买只解锁桃花运容量；涉及校规修改券、星光点兑换券、特殊地点准入证、课程表魔改券或其他物品奖励/消耗时必须写`/系统/持有物品`；特殊地点准入证购买或赠送成功写入/系统/持有物品中的对应准入证，不写/系统/特殊地点解锁；涉及购买VIP成功时必须写`/系统/催眠APP订阅等级`为目标等级；涉及成功打工时必须写`/系统/_社畜值`增加并封顶200，打工buff由前端维护；不能漏掉资源结算。",
      "中文结算要求：成功的催眠APP操作如果有`MC能量消耗`，必须写`/系统/MC能量`扣除；涉及金钱奖励、购买或补给时必须写`/系统/持有零花钱`增减；涉及购买VIP3-6附加星光点、成就/任务奖励、监控派遣收益、邂逅购买角色包、邂逅随机桃花运、邂逅指定角色桃花运、邂逅商店兑换、邂逅商店特殊地点准入证购买、课程表魔改券购买或废止初始校规时必须写`/系统/星光点`增减；角色包浏览不写星光点增减，角色包购买只解锁桃花运容量；涉及校规修改券、星光点兑换券、特殊地点准入证、课程表魔改券或其他物品奖励/消耗时必须写`/系统/持有物品`；特殊地点准入证购买或赠送成功写入/系统/持有物品中的对应准入证，不写/系统/特殊地点解锁；涉及购买VIP成功时必须写`/系统/催眠APP订阅等级`为目标等级；普通招工软件的打工工资、社畜值、buff、buff结束时间和打工后时间由前端直接结算并锁定暂存，AI只承认本轮打工事实，不得再次写入或改写`/系统/持有零花钱`、`/系统/_社畜值`、`/系统/_buff`、`/系统/_buff结束时间`；没有前端打工操作时不要自行结算打工资源。打工/零工模块不是催眠APP的一部分，`/系统/_buff`也不是角色催眠效果；不能漏掉资源结算。"
      )
      .replaceAll("当前MC点", "持有零花钱")
      .replaceAll("奖励MC点", "奖励星光点");
    ...
    next = next.replace(
      /\n-\s*时间\/地点\/事件更新要求：[^\n]*日期和星期写在`\/系统\/当前日期`。[^\n]*/g,
        "\n- 时间/地点/事件更新要求：凡正文、结算摘要或用户输入中出现明确时间推进、转场、当前位置或当前事件变化，必须在同一次<update>里写`/系统/当前日期`、`/系统/当前时间`、`/系统/当前地点`、`/系统/当前事件`的最终值；只要写`/系统/当前时间`，同一次<update>就必须写`/系统/当前日期`，即使日期没有变化。当前时间只写`HH:MM`，`/系统/当前日期`只写日期（如`4月9日`），不要把星期写进当前日期。`/系统/_当前周几`、`/系统/_当前日程`、`/系统/_当前特殊日期`和`/系统/_课程表`是前端只读同步字段，AI不要手写；前端会按当前日期/时间每层同步一次。上述系统时间/地点/事件字段已存在用`replace`，字段不存在先用`add`；这条`add`许可只限这些系统字段，不适用于`/角色`。只写英文System variables摘要、Time passed摘要或裸JSON Patch数组都不算完成变量更新。"
    );
  ...
  patchEntry(entries, "[mvu_plot]人物列表", (content) => content
    ...
    .replaceAll(
      "`事件记录`为6位0/1字符串，前五位记录角色事件1-5是否已触发，第六位记录好感度>=200且服从度>=100的用户自定义事件是否已触发；`工作价值`",
      "`至关重要记忆`为前端只读当前回忆焦点，默认空字符串，只由人物档案回忆按钮写入；事件触发占位由人物档案前端按需写入，不要求自定义角色初始变量预置事件记录字段；`工作价值`"
    )
    .replace(
    ...
    .replace(
      "变量结构需包含`档案`(姓名、年龄、社团/职业、身高、体重、三围、头发、面部、上衣、下衣)、`心理`(此刻想法)、`绰号`(默认空字符串)、`绰号已认可:false`、核心数值、敏感度、次数、临时/永久催眠效果；其中上衣/下衣分别记录上半身/下半身当前可见状态，包含衣物与未被衣物覆盖的肌肤，必要时可保留NSFW细节。",
      "变量结构需包含`档案`(姓名、年龄、社团/职业、身高、体重、三围、头发、面部、上衣、下衣)、`心理`(此刻想法)、`是否派遣中:false`、`工作价值`、`绰号`(默认空字符串)、`绰号已认可:false`、`至关重要记忆:\"\"`、核心数值、敏感度、次数、临时/永久催眠效果；事件触发占位由人物档案前端按需写入，不要求自定义角色初始变量预置事件记录字段；`至关重要记忆`为前端只读当前回忆焦点，默认空字符串，只由人物档案回忆按钮写入；`工作价值`单位为星光点/日，按人设、身份、能力、资源、社会价值、派遣变现潜力和剧情定位生成；生成后固定，后续剧情变化不重算工作价值；核心数值范围-200到200，部位敏感度范围0到1000；其中上衣/下衣分别记录上半身/下半身当前可见状态，包含衣物与未被衣物覆盖的肌肤，必要时可保留NSFW细节。"
    )
    .replace(
      "变量结构需包含`档案`(姓名、年龄、社团/职业、身高、体重、三围、头发、面部、上衣、下衣)、`心理`(此刻想法)、`是否派遣中:false`、`身价`、`绰号`(默认空字符串)、`绰号已认可:false`、核心数值、敏感度、次数、临时/永久催眠效果；`身价`按人设、身份、能力、资源、社会价值和剧情定位生成；其中上衣/下衣分别记录上半身/下半身当前可见状态，包含衣物与未被衣物覆盖的肌肤，必要时可保留NSFW细节。",
  ...
  patchEntry(entries, "[mvu_plot]犬冢夏美人设", (content) => content
    .replace(/age:\s*16\b/g, "age: 17")
  );
    ...
    if (!/\n\s*星光点:\s*/.test(next)) {
      next = next.replace(
	    ...
	    next = next.replace(/\n\s*(?:历史消耗记录|累计消耗MC点|_累计消耗MC点|已花费钞票):\s*[^\n]*/g, "");
		    ...
		    const defaultDailyTimetableBlock = `  _课程表:
    - {课节: 1限, 科目: 英语, 是否魔改: false, 魔改课程: ""}
    - {课节: 2限, 科目: 世界史, 是否魔改: false, 魔改课程: ""}
    - {课节: 3限, 科目: 生物, 是否魔改: false, 魔改课程: ""}
    - {课节: 4限, 科目: 现代文, 是否魔改: false, 魔改课程: ""}
