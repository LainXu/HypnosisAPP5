...
- 魔改课程表保存在前端本地，只影响前端之后同步出的\`/系统/课程表\`、\`/系统/_当前日程\`等只读日程字段。AI不要手写、覆盖或回滚这些只读字段。
- \`/系统/课程表\`只由前端同步，供AI识别当天课表和哪些格子被修改。每行\`科目\`为原始学校课表，\`是否魔改\`为布尔值，\`魔改课程\`为空字符串表示未魔改；若\`是否魔改:true\`，实际课程按\`魔改课程\`理解。已被魔改过的格子可在课程表APP里随时重命名，不再额外消耗课程表魔改券；新增魔改格子仍需消耗课程表魔改券，且每张券只能修改一个当天剩余课节。
...
- 固定物品描述：\`课程表魔改券\`是邂逅商店特权道具，VIP6用户可用100星光点购买1张；课程表APP保存当天剩余课程中的一节修改时消耗1张，课程表内容由前端本地保存，并同步\`/系统/课程表\`只读日程字段。
...
- 相关变量含义：启动/追加催眠给\`MC能量\`；购买VIP给\`持有零花钱\`、\`星光点\`和\`催眠APP订阅等级\`；补充MC能量给\`持有零花钱\`、\`MC能量\`和\`MC能量上限\`；提升MC能量上限给\`持有零花钱\`和\`MC能量上限\`；领取成就/任务奖励给\`星光点\`和\`持有物品\`；邂逅随机桃花运使用后由前端扣\`星光点\`、创建初始\`角色\`变量并写入当前对话独有的Chat Lorebook，指定角色包/单独角色仅浏览不扣费不写变量；邂逅商店VIP5可进入，VIP5及以上且有\`星光点兑换券\`时可按10000零花钱兑换1星光点，VIP5可用20星光点购买特殊地点准入证，VIP6才可用100星光点兑换1张\`校规修改券\`或购买1张\`课程表魔改券\`；特殊地点准入证和课程表魔改券给\`星光点\`和\`持有物品\`；打工通常只给\`持有零花钱\`，若本次打工触发\`全盛出击\`才额外给\`MC能量\`和\`MC能量上限\`；监控派遣角色给\`主角可疑度\`；监控派遣结束/取消给\`星光点\`；申请校规给\`校规修改券\`、VIP等级和当前校规数；废止初始校规则给\`星光点\`、VIP等级和当前校规数。\`社畜值\`、\`buff\`、\`buff结束时间\`、地点/事件和只读日程由前端维护，打工提示词通常不放入\`<相关变量>\`。
- \`/系统/_当前周几\`、\`/系统/_当前日程\`、\`/系统/_当前特殊日期\`和\`/系统/课程表\`等前端只读派生字段通常不放进\`<相关变量>\`；除非本轮操作正是课程表魔改或事件触发必须对比课表，否则AI只读它们，不手写、不回滚、不要求每次携带。
...
- 邂逅中的指定角色包和单独角色已改为仅浏览资料，不再购买、不扣星光点、不创建角色变量、不写入世界书。只有随机桃花运仍消耗6星光点，并由前端随机抽中角色、创建初始变量、写入当前对话独有的Chat Lorebook；AI只需要按本轮邂逅提示安排桃花运剧情，不再创建、重建或补写角色/世界书。
...
- 邂逅APP的\`角色包已使用\`现在只应由随机桃花运产生，表示前端已经完成随机购买确认、扣除\`/系统/星光点\`、创建初始\`/角色\`变量、缓存角色图片，并尝试把命中角色世界书内容写入当前对话独有的Chat Lorebook；指定角色包/单独角色浏览不会产生此操作。随机桃花运由前端先从带世界书内容且当前对话未导入的角色中随机抽中1名，AI不得重新随机或替换命中角色。该世界书只绑定当前对话，不应污染同一卡的其他对话；世界书插入不可撤销，若需要撤销只能由用户之后手动删除；变量可随楼层回滚。同一轮最多处理一个随机桃花运，若异常出现多条，只处理最早一条。AI收到该操作时不要重复扣星光点，不要重复插入世界书，也不要创建、重建或补写角色/世界书；只根据角色包信息、整体邂逅提示、已写入世界书和当前剧情安排对应角色登场，并在后续按剧情变化更新\`/角色\`变量。已存在角色只补缺失字段或在正文说明冲突。
...
规则:
- 邂逅中的指定角色包和单独角色仅作为资料浏览、定制、导入导出和世界书管理入口，不消耗星光点，不创建角色变量，不把角色条目写入当前对话Chat Lorebook。
...
- 随机桃花运由前端从带世界书内容且当前对话未导入的角色中随机抽中1名，然后扣星光点、创建初始变量并写入当前对话Chat Lorebook；若第一次购买且当前对话没有绑定世界书，前端会先创建并绑定。AI收到“随机命中角色”后不得重新抽、不得换人、不得把未命中的角色加入。
- 星光点是APP内部货币，邂逅登场角色不知道星光点、不能支付星光点、不能返还星光点，也不能把自己的人脉/金钱/资源兑换成星光点；若角色愿意支持{{user}}，只能提供剧情资源、零花钱、物品、场地、人脉或情报。
- 角色包已使用现在只应对应随机桃花运命中的角色：前端已经完成随机购买确认、扣除/系统/星光点、创建初始/角色变量、缓存角色图片，并尝试把命中角色世界书内容写入当前对话独有的Chat Lorebook。指定角色包/单独角色浏览不会产生这个操作。
...
- 前端会读取当前对话Chat Lorebook条目名判断随机桃花运重复；若某个角色的[mvu_update]角色名变量或[mvu_plot]角色名人设已经存在，随机抽取会避开或跳过。AI收到已跳过名单时只沿用已有角色状态，不要覆盖已有角色。
...
- VIP6可用100星光点购买1张课程表魔改券；课程表APP保存当天剩余课程中的一节修改时消耗1张，课程表内容由前端本地保存，并同步\`/系统/课程表\`只读日程字段；\`课程表\`每行保留原本\`科目\`，用\`是否魔改\`和\`魔改课程\`记录实际改动。
...
- AI应按剧情合理性、地点权限和现实阻碍决定是否移动/转场；若成立，更新/系统/当前地点和/系统/当前事件；若不成立，保持变量不变并在正文说明。/系统/_当前周几、/系统/_当前日程、/系统/_当前特殊日期和/系统/课程表是前端只读同步字段，AI不要手写。
...
- 日历/时钟展示本身不是自动推进时间；只有本轮操作、剧情行动、打工、派遣结算或AI叙事明确推进时才由AI更新/系统/当前日期和/系统/当前时间。/系统/当前日期只写日期；/系统/_当前周几、/系统/_当前日程、/系统/_当前特殊日期和/系统/课程表是前端只读同步字段，AI不要手写。
</APP操作-档案与杂项>`;
...
function patchScheduleWorldbookMentions(content) {
  ...
  if (next.includes("手机主界面会根据`系统.当前日期`、`系统.当前时间`和`系统._当前日程`静态显示星期与当前课段。")) {
    next = next.replace(
      ...
      "手机主界面会根据`系统.当前日期`、`系统.当前时间`和前端只读字段`系统._当前周几`、`系统._当前日程`、`系统._当前特殊日期`、`系统.课程表`静态显示星期、当前课段与课程表。"
    );
  }
  if (!next.includes("_当前周几") && next.includes("_当前日程")) {
    next += "\n- `/系统/_当前周几`、`/系统/_当前日程`、`/系统/_当前特殊日期`和`/系统/课程表`都是前端只读同步字段，AI不要手写。";
  }
  return next;
}
...
const CharacterStats = z.intersection(`
  ...
  next = next.replace(/\n\s*(?:["']当前或下个特殊日期["']|当前或下个特殊日期):\s*z[^\n]*/g, "");
    ...
    next = next.replace(
      timetableAnchor,
	      `$&
	    课程表: z.array(z.any()).prefault([]),`
	    );
	  }
  if (!/当前事件:\s*z/.test(next)) {
	    const eventAnchor = /^    课程表:.*$/m.test(next)
	      ? /^    课程表:.*$/m
	      : /^    _当前特殊日期:.*$/m.test(next)
	          ? /^    _当前特殊日期:.*$/m
        : /^    _当前日程:.*$/m.test(next)
...
function migrateWorkbenchAlternateGreetingDefaults(workbench) {
  ...
  system["课程表"] = defaultDailyTimetableRows.map((row) => ({ ...row, 是否魔改: false, 魔改课程: "" }));
  delete system["当前日程"];
  for (const key of ["当前" + "/" + "待上课程", "当前或" + "待上课程", "当前或" + "下个特殊日期"]) {
    delete system[key];
  }
...
function patchCard(card) {
  ...
  patchEntry(entries, "[mvu_update]变量更新格式", (content) => {
    ...
    next = next.replace(
      /\n-\s*时间\/地点\/事件更新要求：[^\n]*日期和星期写在`\/系统\/当前日期`。[^\n]*/g,
        "\n- 时间/地点/事件更新要求：凡正文、结算摘要或用户输入中出现明确时间推进、转场、当前位置或当前事件变化，必须在同一次<update>里写`/系统/当前时间`、`/系统/当前地点`、`/系统/当前事件`的最终值；当前时间只写`HH:MM`，`/系统/当前日期`只写日期（如`4月9日`），不要把星期写进当前日期。`/系统/_当前周几`、`/系统/_当前日程`、`/系统/_当前特殊日期`和`/系统/课程表`是前端只读同步字段，AI不要手写；前端会按当前日期/时间每层同步一次。上述系统时间/地点/事件字段已存在用`replace`，字段不存在先用`add`；这条`add`许可只限这些系统字段，不适用于`/角色`。只写英文System variables摘要、Time passed摘要或裸JSON Patch数组都不算完成变量更新。"
    );
  ...
  patchEntry(entries, "[initvar]变量初始化不需要开", (content) => {
    ...
    if (!/\n\s*阿宅性别:\s*/.test(next)) {
      next = next.replace(
	    ...
	    const defaultDailyTimetableBlock = `  课程表:
    - {课节: 1限, 科目: 英语, 是否魔改: false, 魔改课程: ""}
    - {课节: 2限, 科目: 世界史, 是否魔改: false, 魔改课程: ""}
    - {课节: 3限, 科目: 生物, 是否魔改: false, 魔改课程: ""}
    - {课节: 4限, 科目: 现代文, 是否魔改: false, 魔改课程: ""}
