...
const REWARD_ITEM_PRESETS = [
  ...
  {
    name: "课程表魔改券",
    description: "邂逅商店特权道具。VIP6用户可用100星光点购买1张；课程表APP保存当天剩余课程中的一节修改时消耗1张，课程表内容由前端本地保存，并同步当天原课程表和当天魔改课程表等只读日程字段。",
  },
];
...
// 不再处理 MC能量、主角可疑度、角色警戒度，也不维护 _当前周几/_当前日程/当天原课程表/当天魔改课程表；这些规则分别由世界书和前端负责。
...
- 魔改课程表保存在前端本地，只影响前端之后同步出的\`/系统/当天魔改课程表\`、\`/系统/_当前日程\`等只读日程字段；\`/系统/当天原课程表\`保留同一天原始学校课表用于对比。AI不要手写、覆盖或回滚这些只读字段。
- \`/系统/当天原课程表\`和\`/系统/当天魔改课程表\`都只由前端同步，供AI识别哪些格子被修改。已被魔改过的格子可在课程表APP里随时重命名，不再额外消耗课程表魔改券；新增魔改格子仍需消耗课程表魔改券，且每张券只能修改一个当天剩余课节。
...
- 固定物品描述：\`课程表魔改券\`是邂逅商店特权道具，VIP6用户可用100星光点购买1张；课程表APP保存当天剩余课程中的一节修改时消耗1张，课程表内容由前端本地保存，并同步当天原课程表和当天魔改课程表等只读日程字段。
...
- \`/系统/_当前周几\`、\`/系统/_当前日程\`、\`/系统/_当前特殊日期\`、\`/系统/当天原课程表\`和\`/系统/当天魔改课程表\`等前端只读派生字段通常不放进\`<相关变量>\`；除非本轮操作正是课程表魔改或事件触发必须对比课表，否则AI只读它们，不手写、不回滚、不要求每次携带。
...
- VIP6可用100星光点购买1张课程表魔改券；课程表APP保存当天剩余课程中的一节修改时消耗1张，课程表内容由前端本地保存，并同步当天原课程表和当天魔改课程表等只读日程字段。
...
- 前端会在点击打工后弹窗选择打工开始时间，并直接写入/系统/持有零花钱、/系统/社畜值、/系统/buff、/系统/buff结束时间、/系统/当前日期、/系统/当前时间、/系统/当前事件，以及/系统/_当前周几、/系统/_当前日程、/系统/_当前特殊日期、/系统/当天原课程表、/系统/当天魔改课程表等只读同步字段。AI不要手写这些前端只读字段，不要重复发钱、增加社畜值、改写buff、恢复MC能量或二次结算打工。
- 打工相关\`<相关变量>\`只携带本次需要AI承认或判断的少量结算状态，例如\`持有零花钱\`、\`buff\`、\`buff结束时间\`；\`社畜值\`和只读日程字段由前端同步，通常不塞进\`<相关变量>\`。只有\`全盛出击\`这类涉及MC能量恢复的打工结果，才额外携带\`MC能量\`和\`MC能量上限\`。
...
- AI应按剧情合理性、地点权限和现实阻碍决定是否移动/转场；若成立，更新/系统/当前地点和/系统/当前事件；若不成立，保持变量不变并在正文说明。/系统/_当前周几、/系统/_当前日程、/系统/_当前特殊日期、/系统/当天原课程表和/系统/当天魔改课程表是前端只读同步字段，AI不要手写。
...
- 日历/时钟展示本身不是自动推进时间；只有本轮操作、剧情行动、打工、派遣结算或AI叙事明确推进时才由AI更新/系统/当前日期和/系统/当前时间。/系统/当前日期只写日期；/系统/_当前周几、/系统/_当前日程、/系统/_当前特殊日期、/系统/当天原课程表和/系统/当天魔改课程表是前端只读同步字段，AI不要手写。
</APP操作-档案与杂项>`;
...
function patchVariableRules(content) {
  ...
  next = next
    .replace(/当前\/待上课程/g, "当天魔改课程表")
    .replace(/当前或待上课程/g, "当天魔改课程表")
...
function patchScheduleWorldbookMentions(content) {
  let next = String(content ?? "")
    ...
    .replace(/AI叙事和变量更新应维护`当前日期`、`当前时间`、`当前日程`、`当前\/待上课程`和`当前事件`，并检查当前场景是否符合课程\/周末\/假期。/g, "AI叙事和变量更新只维护`当前日期`、`当前时间`、`当前地点`和`当前事件`；`_当前周几`、`_当前日程`、`_当前特殊日期`、`当天原课程表`和`当天魔改课程表`由前端只读同步，AI不要手写。")
  ...
  if (next.includes("手机主界面会根据`系统.当前日期`、`系统.当前时间`和`系统._当前日程`静态显示星期与当前课段。")) {
    next = next.replace(
      ...
      "手机主界面会根据`系统.当前日期`、`系统.当前时间`和前端只读字段`系统._当前周几`、`系统._当前日程`、`系统._当前特殊日期`、`系统.当天原课程表`、`系统.当天魔改课程表`静态显示星期、当前课段与课程表对比。"
    );
  }
  if (!next.includes("_当前周几") && next.includes("_当前日程")) {
    next += "\n- `/系统/_当前周几`、`/系统/_当前日程`、`/系统/_当前特殊日期`、`/系统/当天原课程表`和`/系统/当天魔改课程表`都是前端只读同步字段，AI不要手写。";
  }
  return next;
}
  ...
  next = next.replace(/\n\s*(?:["']当前或下个特殊日期["']|当前或下个特殊日期):\s*z[^\n]*/g, "");
    ...
    next = next.replace(
	  ...
	  if (!/(?:["']当天魔改课程表["']|当天魔改课程表):\s*z/.test(next)) {
	    next = next.replace(
	      /^    当天原课程表:.*$/m.test(next) ? /^    当天原课程表:.*$/m : /^    _当前特殊日期:.*$/m,
	      `$&
...
function migrateWorkbenchAlternateGreetingDefaults(workbench) {
  ...
  delete system["当天课程表"];
  system["当天原课程表"] = defaultDailyTimetableRows.map((row) => ({ ...row }));
  system["当天魔改课程表"] = defaultDailyTimetableRows.map((row) => ({ ...row }));
  delete system["当前日程"];
  for (const key of ["当前" + "/" + "待上课程", "当前或" + "待上课程", "当前或" + "下个特殊日期"]) {
    delete system[key];
  }
...
function patchCard(card) {
  ...
  patchEntry(entries, "[mvu_update]变量更新格式", (content) => {
    ...
    next = next.replace(
      /\n-\s*时间\/地点\/事件更新要求：[^\n]*日期和星期写在`\/系统\/当前日期`。[^\n]*/g,
      "\n- 时间/地点/事件更新要求：凡正文、结算摘要或用户输入中出现明确时间推进、转场、当前位置或当前事件变化，必须在同一次<update>里写`/系统/当前时间`、`/系统/当前地点`、`/系统/当前事件`的最终值；当前时间只写`HH:MM`，`/系统/当前日期`只写日期（如`4月9日`），不要把星期写进当前日期。`/系统/_当前周几`、`/系统/_当前日程`、`/系统/_当前特殊日期`、`/系统/当天原课程表`和`/系统/当天魔改课程表`是前端只读同步字段，AI不要手写；前端会按当前日期/时间每层同步一次。上述系统时间/地点/事件字段已存在用`replace`，字段不存在先用`add`；这条`add`许可只限这些系统字段，不适用于`/角色`。只写英文System variables摘要、Time passed摘要或裸JSON Patch数组都不算完成变量更新。"
    );
  ...
  patchEntry(entries, "[initvar]变量初始化不需要开", (content) => {
    ...
    if (!/\n\s*阿宅性别:\s*/.test(next)) {
      next = next.replace(
	    ...
	    const defaultDailyTimetableBlock = `  当天原课程表:
    - {课节: 1限, 科目: 英语}
    - {课节: 2限, 科目: 世界史}
    - {课节: 3限, 科目: 生物}
    - {课节: 4限, 科目: 现代文}
