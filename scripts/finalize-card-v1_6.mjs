...
- 新增地点操作只用于维护前端localStorage地点列表；前端已经按用户输入保存地点，并只在暂存中提供少量典型示例。AI不要输出完整地图JSON，不要把新增地点误写成MVU变量；只需承认该地点存在，信息不足时可按剧情补足用途、氛围、关联角色或分类。除非用户明确要求批量重排地图，否则不要输出\`<地图更新>\`或\`<学校地图更新>\`完整列表。
...
- 新增地点操作只用于维护前端localStorage地点列表；前端已经按用户输入保存地点，并只在暂存中提供少量典型示例。AI不要输出完整<地图更新>或<学校地图更新> JSON，不要把新增地点误写成MVU变量；只需承认该地点存在，信息不足时可按剧情补足用途、氛围、关联角色或分类。
...
- 用户后续新增地点只维护前端地图列表；前端按用户输入保存并在暂存区提醒AI承认地点存在。AI不要输出完整\`<地图更新>\`或\`<学校地图更新>\` JSON，不要把新增地点误写成MVU变量；信息不足时可在剧情里自然补足地点用途、氛围或分类，也不要用新增地点绕过准入证、门禁、时间或剧情阻碍。
