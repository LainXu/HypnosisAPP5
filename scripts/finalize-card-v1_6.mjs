...
- 对人物档案中的敏感度、次数、临时/永久催眠效果等角色字段，只在剧情或操作结算明确造成变化时更新；不得把展示文本当作已发生事实。角色只要在本次AI回复中出现、说话、行动或与任何人互动，就必须同步更新该角色\`心理\`为此刻短句想法，即使其他数值不变也不能沿用过期心理。人物档案的删除催眠效果按钮只请求删除指定角色、指定类型下的单个效果；成功时只能remove对应\`/角色/角色名/临时催眠效果/效果名\`或\`/角色/角色名/永久催眠效果/效果名\`这一条目标路径。删除催眠效果不是废止校规，严禁remove \`/校规\`、\`/系统\`、其他角色、其他效果或角色数值；若本轮操作给出\`目标变量路径\`，必须以该路径为准。
...
- 人物档案删除催眠效果只按[APP操作-档案与杂项]结算，只能remove本轮操作给出的角色催眠效果目标路径；这不是删除校规，严禁remove \`/校规\`。
...
- 人物档案的删除催眠效果按钮只请求删除指定角色、指定类型下的单个效果；成功时只能remove对应/角色/角色名/临时催眠效果/效果名或/角色/角色名/永久催眠效果/效果名这一条目标路径，不要顺手改其他字段。删除催眠效果不是删除校规或废止规则，严禁remove /校规、/系统、其他角色、其他效果、好感度、服从度、敏感度或次数；如果输出remove /校规/出勤学习、/校规/仪容礼仪等校规路径，就是错误变量更新，必须改成目标催眠效果路径或在正文说明失败。
...
const CharacterStats = z.intersection(`
  ...
  if (!/_事件记录:\s*z\./.test(next)) {
    next = next.replace(
      /^    绰号已认可:.*$/m,
      `$&
    _事件记录: z.coerce.string().prefault('000000').transform(v => /^[01]{6}$/.test(String(v).trim()) ? String(v).trim() : '000000'),`
    );
  }
  next = next.replace(
    /^    _事件记录:\s*z[^\n]*/m,
  ...
  if (!/事件记录:\s*z\./.test(next)) {
    next = next.replace(
      /^    _事件记录:.*$/m,
      `$&
    事件记录: z.coerce.string().prefault('000000').transform(v => /^[01]{6}$/.test(String(v).trim()) ? String(v).trim() : '000000'),`
    );
  }
  next = next.replace(
    /^    事件记录:\s*z[^\n]*/m,
...
function patchCard(card) {
  ...
  patchEntry(entries, "[mvu_plot]人物列表", (content) => content
    ...
    .replaceAll(
      "`事件记录:\"000000\"`、核心数值",
      "`_事件记录:\"000000\"`、`事件记录:\"000000\"`、`至关重要记忆:\"\"`、核心数值"
    )
    .replaceAll(
      "`事件记录`为6位0/1字符串，前五位记录角色事件1-5是否已触发，第六位记录好感度>=200且服从度>=100的用户自定义事件是否已触发；`工作价值`",
      "`_事件记录`为前端只读维护的6位0/1占位字符串，前五位记录角色事件1-5是否已触发，第六位记录好感度>=200且服从度>=100的用户自定义事件是否已触发；`事件记录`仅旧版兼容；`至关重要记忆`为前端只读当前回忆焦点，默认空字符串，只由人物档案回忆按钮写入；`工作价值`"
    )
    .replace(
    ...
    .replace(
      ...
      "变量结构需包含`档案`(照片、姓名、年龄、社团/职业、身高、体重、三围、头发、面部、上衣、下衣)、`心理`(此刻想法)、`是否派遣中:false`、`工作价值`、`绰号`(默认空字符串)、`绰号已认可:false`、`_事件记录:\"000000\"`、`事件记录:\"000000\"`、`至关重要记忆:\"\"`、核心数值、敏感度、次数、临时/永久催眠效果；`_事件记录`为前端只读维护的6位0/1占位字符串，前五位记录角色事件1-5是否已触发，第六位记录好感度>=200且服从度>=100的用户自定义事件是否已触发；`事件记录`仅旧版兼容；`至关重要记忆`为前端只读当前回忆焦点，默认空字符串，只由人物档案回忆按钮写入；`工作价值`单位为星光点/日，按人设、身份、能力、资源、社会价值、派遣变现潜力和剧情定位生成；生成后固定，后续剧情变化不重算工作价值；核心数值范围-200到200，部位敏感度范围0到1000；其中上衣/下衣分别记录上半身/下半身当前可见状态，包含衣物与未被衣物覆盖的肌肤，必要时可保留NSFW细节。"
    )
    .replace(
