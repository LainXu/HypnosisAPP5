...
- 角色根字段\`事件记录\`是前端只读维护的5位字符串，默认\`00000\`，从左到右表示该角色事件1到事件5是否已触发；事件1触发显示为\`10000\`，事件2触发把第二位改为1，依此类推。用户在人物档案事件页选择事件时，前端会直接写入对应位并锁定暂存；AI只根据本轮\`触发角色事件\`操作生成对应事件剧情，不得手写、补写、回退、清空或自行replace这个字符串。触发事件的回复末尾必须额外输出\`<人物档案事件记录>\`块，包含角色名、事件序号、标题、概要、关键场面、关系变化和后续钩子；该块只供前端本地保存回忆摘要，不写入MVU变量。没有前端事件操作时，不要自行触发编号事件。
...
- \`事件记录\`是角色个人事件位图，格式固定5位\`0/1\`字符串，由前端只读维护；用户在人物档案事件页选择事件时前端直接改对应位并锁定暂存。AI只根据该操作写事件剧情，不得手写、补写、回退、清空或自行replace事件记录；触发事件的回复末尾必须输出\`<人物档案事件记录>\`块，字段包含角色名、事件序号、标题、概要、关键场面、关系变化和后续钩子，供前端本地保存为回忆摘要，不写入MVU变量；没有前端事件操作时，不要自行触发编号事件。
...
- 事件正式触发后，回复末尾输出\`<人物档案事件记录>\`块，简洁记录足够日后回忆的事件标题、概要、关键场面、关系变化和后续钩子；该块只供前端本地收录，不写入MVU。
...
- 事件贰:
  好感阈值: 70
  事件名: 三人漫展邀请
  事件描述: 爱丽莎邀请{{user}}和阿宅一起去漫展。她对阿宅仍有青梅竹马式的熟悉与指挥感，对{{user}}则表现出把他纳入私人兴趣圈的信任；是否三人同行、临时改期或被旁人打断由剧情决定。
- 事件叁:
  好感阈值: 100
  事件名: 巧克力与准入证
...
- 事件伍:
  好感阈值: 200
  事件名: 全班面前的公开恋情
  事件描述: 爱丽莎在班上所有人面前宣布和{{user}}的恋情并亲吻{{user}}。这应是高度公开、影响班级关系和阿宅反应的大事件；若当前剧情不适合立刻发生，可先铺垫，但不要改成私下小事。
</西园寺爱丽莎好感事件链>`;
