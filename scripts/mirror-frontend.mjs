...
\`;
  ...
  function clearPhoneInternalOverlays(root) {
    root?.querySelectorAll?.(".st-mchan-internal-app,.st-add-role-app,.st-calendar-lite-app,.st-timetable-app").forEach((element) => element.remove());
  }
  ...
  function looksLikePhoneHome(root) {
    ...
    return ["催眠APP", "日历", "帮助", "成就和任务", "库存", "MC匿名版"].every((label) => rootText.includes(label)) &&
      !root.querySelector(".st-mchan-internal-app, .st-add-role-app, .st-calendar-lite-app, .st-timetable-app");
  }
  ...
  function patchHomeTile() {
    ...
    patchAddRoleTile();
    const labels = Array.from(document.querySelectorAll("span,button,div"))
      .filter((element) => !element.closest(".st-mchan-internal-app,.st-add-role-app,.st-calendar-lite-app,.st-timetable-app") && element.textContent?.trim() === "MC匿名版");
    for (const label of labels) {
      const tile = findTileFromLabel(label);
  ...
  function hasInitialPhonePatch() {
    ...
    if (document.querySelector('[data-st-add-role-tile="true"], [data-st-timetable-tile="true"], .st-role-picker, .st-mchan-internal-app, .st-add-role-app, .st-calendar-lite-app, .st-timetable-app')) return true;
    return false;
  }
