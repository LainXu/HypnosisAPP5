...
.st-person-empty{border:1px dashed rgba(81,64,49,.38);border-radius:10px;background:rgba(247,241,229,.9);padding:22px 14px;text-align:center;color:rgba(68,54,42,.64);font-size:13px;line-height:1.55}
  ...
  const STATIC_GRAPH_DEFAULTS = {
    world: {
      title: "区域地图",
      nodes: [
        { id: "school", label: "私立斋明学园", kind: "world", x: 50, y: 45 },
        { id: "saionji-company", label: "西园寺企业", kind: "world", x: 77, y: 23 },
        { id: "saionji-home", label: "西园寺的家", kind: "home", x: 78, y: 70 },
        { id: "miyuki-home", label: "深雪的家", kind: "home", x: 23, y: 70 },
        { id: "natsumi-home", label: "夏美的家", kind: "home", x: 22, y: 25 }
      ],
    ...
    school: {
      title: "学校地图",
      nodes: [
        { id: "classroom", label: "教室", kind: "school", x: 50, y: 28 },
        { id: "corridor", label: "走廊", kind: "school", x: 50, y: 50 },
        { id: "library", label: "图书馆", kind: "school", x: 23, y: 42 },
        { id: "principal", label: "校长室", kind: "school", x: 24, y: 68 },
        { id: "pool", label: "游泳池", kind: "school", x: 77, y: 33 },
        { id: "field", label: "操场", kind: "school", x: 76, y: 66 }
      ],
  ...
  function parseStaticGraphUpdateText(text) {
    ...
    if (!source.includes("<地图更新>") && !source.includes("<学校地图更新>")) return;
    const specs = [
  ...
  function syncStaticGraphUpdatesFromChat() {
    ...
    for (const message of messages) {
      const body = String(message?.message || "");
      const key = String(message?.message_id ?? message?.id ?? "") + ":" + body.length;
      if (seen.has(key)) continue;
      seen.add(key);
      parseStaticGraphUpdateText(body);
    }
  }
  ...
  function renderStaticGraphCard(scope) {
    ...
    const nodes = graph.nodes.map((node) => (
      '<button type="button" class="st-graph-node ' + escapeAttr(node.kind) + '" style="left:' + node.x + '%;top:' + node.y + '%" title="' + escapeAttr(node.label) + '">' +
    ...
    return '<section class="st-lite-card st-graph-card">' +
      '<div class="st-graph-head"><strong>' + escapeHtml(graph.title) + '</strong><span>' + graph.nodes.length + ' 地点 / ' + graph.edges.length + ' 连接</span></div>' +
  ...
  function renderSchoolRulesCard() {
    ...
    const list = entries.length
      ? entries.map(([name, value]) => {
          ...
          return '<article class="st-rule-item"><strong>' + escapeHtml(name) + '</strong><p>' + escapeHtml(text) + '</p></article>';
        }).join("")
    ...
    '<section class="st-lite-card st-rule-form">' +
      ...
      '<div class="st-rule-cost"><span>¥1,000,000</span><span>MC能量 10000</span><span>MC点 10000</span></div>' +
      '<button class="st-rule-button" type="button" data-school-rule-submit>申请立校规</button>' +
    '</section>';
  }
  ...
  function bindGraphPageActions(page) {
    page.querySelectorAll("[data-graph-action]").forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (button.getAttribute("data-graph-action") === "reset") resetStaticGraph(scope);
    ...
    page.querySelector("[data-school-rule-submit]")?.addEventListener("click", () => {
      ...
      appendAppOperation({
        ...
        AI执行规范: "余额全部足够时扣除三项代价并在/校规中新增；任一不足则失败，不扣费不生效。"
      });
      input.value = "";
    });
  }
