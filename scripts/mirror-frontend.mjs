...
\`;
  ...
  function shouldUseScanExampleFallback(example, index) {
    if (index !== 2 || !example || typeof example !== "object" || Array.isArray(example)) return false;
  ...
  function patchPhoneAuthorCredit(root) {
    ...
    const directText = (element) => Array.from(element.childNodes || [])
      ...
      .replace(/\s+/g, " ")
      .trim();
    const writeDirectText = (element) => {
      if (directText(element) === ST_AUTHOR_CREDIT) {
        element.classList.add("st-author-status-credit");
        return;
      }
      const textNodes = Array.from(element.childNodes || []).filter((node) => node.nodeType === Node.TEXT_NODE && /^\\s*\\d{1,2}[:：]\\d{2}\\s*$/.test(String(node.nodeValue || "")));
      if (textNodes.length) {
        textNodes[0].nodeValue = ST_AUTHOR_CREDIT;
    ...
    const candidates = Array.from(root.querySelectorAll("span,div,p"))
      .filter((element) => {
        ...
        const text = ownText || (element.children.length ? "" : compactText(element));
        ...
        return top >= 0 && top < 82 && left >= 0 && left < 230 && fontSize <= 34;
      });
    for (const element of candidates) {
      writeDirectText(element);
    }
  }
