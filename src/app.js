...
function patchOriginalWorldbookForMergedPolicy(entries) {
  ...
  patchEntryContent(entries, "[mvu_plot]强调要求", (content) => {
    ...
    if (!next.includes(bodyStatsLine)) {
      next = next.replace(
	    ...
	    const resourceGlossaryLine = "- 资源名必须严格区分：`系统.MC能量`是催眠功能实际消耗的能量余额；`系统.MC能量上限`只是能量容量上限，不是可花费余额；`系统.持有零花钱`是金钱余额。当前版本只保留金钱与MC能量两类可结算资源。";
  ...
  patchEntryContent(entries, "[mvu_plot]催眠指导", (content) =>
    ensureLinesBeforeEndTag(
      ...
      [
		        "- 新前端会在启动催眠和每条催眠功能里写明前端已计算好的`预计消耗`与总`MC能量消耗`；AI不要重新套人数、部位或时间公式，只需检查余额、权限、目标状态、风险与最终是否成功。",
	        "- 所有涉及花费的催眠APP功能在生效前必须逐项检查余额：`系统.MC能量`支付能量费用，`系统.持有零花钱`支付金钱费用；余额不足则该功能失败，不产生催眠效果，也不得扣成负数。",
	        "- 同一批次内后续依赖失败功能、启动催眠成功状态或同一资源余额的操作，若受余额不足影响也必须失败；AI不能贷款、透支、自动补给、自动把金钱兑换成MC能量，除非`本轮APP操作`明确包含对应兑换/补给且该兑换本身费用充足。",
  ...
  patchEntryContent(entries, "[mvu_update]变量说明和更新规则🈯", (content) => {
	    const resourceRuleBlock = [
	      ...
	      "      check:",
	      "        - 普通催眠消耗不会改变此值；只有明确升级、扩容或规则说明时才更新。",
    ...
    const next = [
      ...
      "- 如果本轮用户输入中没有<本轮APP操作>容器，或容器为空/无，则代表{{user}}没有操作APP，严禁进行相关新增操作描写。",
	      "- 前端只记录用户在手机界面里的操作意图，不直接发送指令，也不直接改最终变量。",
		      "- AI必须根据剧情、资源/金钱、VIP权限、人数、时间、目标状态、风险和合理性判断操作是否成功。",
  ...
  patchEntryContent(entries, "[mvu_update]匿名版介绍", (content) => {
    const staticBlock = [
      ...
      "  - 若正文剧情明确让{{user}}接取悬赏或发布内容，再按任务/奖励规则结算；不能因为静态展示或浏览动作自动添加任务、收益或变量。"
    ].join("\n");
