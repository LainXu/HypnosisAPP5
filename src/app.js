  ...
  patchEntryContent(entries, "[mvu_update]本轮APP操作", (content) => {
    const next = [
      ...
      "- 资源名必须严格区分：`MC能量`=催眠能量余额；`MC能量上限`=容量上限，不可花费；`当前MC点`=PT/MC点货币；`持有零花钱`=金钱。不同资源不能互相顶替。",
	      "- 订阅、资源兑换、启动/追加催眠、申请校规、废止初始校规、领取奖励等操作会在`本轮APP操作`里携带`当前变量余额`；它只包含当前按钮实际需要检查/消耗/增减的变量：订阅/补给/购买当前MC点只给`持有零花钱`，提升MC能量上限只给`当前MC点`，启动/追加催眠只给`MC能量`，领取成就/任务奖励只给`当前MC点`，申请/废止初始校规则给`当前MC点`、订阅等级、爱丽莎好感度和当前校规数。未出现在余额快照里的资源不要自行脑补为可用余额。",
      ...
      "- 申请/发布/删除/废止校规只按[mvu_update]校规规则结算；校规只写入`/校规`，不要写入角色临时/永久催眠效果。",
