    ...
    const resourceRuleBlock = [
      ...
      "        - 只有当本轮剧情明确展示或改变角色当下想法时更新；内容应贴合当前场景、目标和短期注意力。",
	    ...
	    const taskRuleBlock = [
	      ...
	      "      }",
	      "    check:",
	      "      - `成就`变量只临时保存已完成且已发放奖励、等待前端同步的成就；不要保存未完成成就。",
	      "      - 静态成就完成并发奖后写入这里，至少包含`奖励MC点`和`已完成:true`；知道成就ID时同时写`成就ID`。",
	      "      - 前端最新楼层会读取一次并写入浏览器本地状态，然后删除变量里的对应成就。",
	      "  任务:",
	      "    type: |-",
	      "      {",
	      "        [任务名: string]: {",
	      ...
	      "      }",
	      "    check:",
	      "      - `任务`变量保存已接/进行中任务，也可临时保存已完成但尚未被前端同步的任务；最多3个进行中任务，静态任务未接取前不写入变量。",
	      ...
	      "      - 当本轮回复满足任务完成条件时，直接把奖励加到`系统/当前MC点`，并在同一次<update>中把该任务保留完整信息且设为`已完成:true`。",
  ...
  patchEntryContent(entries, "[mvu_update]本轮APP操作", (content) => {
    const next = [
      ...
      "- 单功能购买已取消：只要对应VIP等级已经订阅/解锁，前端允许直接启用该等级内功能；AI不需要写入或维护任何`购买状态`变量。",
	      ...
	      "- `成就`变量只作为“待前端同步的已完成成就”临时容器；不要保存未完成成就。前端同步后会清空对应条目。",
      ...
      "- 任务完成后不等待用户点击领取：若本轮剧情满足某个任务完成条件，AI必须在同一次<update>中把奖励加到`/系统/当前MC点`，并把`/任务/任务名`改为保留任务ID/完成条件/奖励MC点且`已完成:true`；不要直接remove。前端最新楼层会只扫描一次已完成任务，写入浏览器本地已完成状态后删除变量里的该任务。",
      "- 静态成就或静态任务奖励成功发放后，不输出旧式前端状态JSON块。改为把完成记录写入变量供前端同步：成就写入`/成就/成就名`（含`成就ID`、条件/描述、`奖励MC点`、`已完成:true`）；静态任务写入`/任务/任务名`（含`任务ID`、`完成条件`、`奖励MC点`、`已完成:true`）。前端遇到未知任务/成就会添加为本地动态已完成项，已有项只更新状态；失败、余额/条件不足或只是接取任务时不要写完成记录。",
