...
function patchOriginalWorldbookForMergedPolicy(entries) {
  ...
  const schoolRuleWorldbook = [
    ...
    "- 废止初始默认校规（`仪容礼仪`、`出勤学习`、`校内安全`）与发布新校规代价相同：必须VIP5、西园寺爱丽莎好感度至少100、`系统.当前MC点`至少500000；成功扣除500000当前MC点并remove对应校规，失败不扣费不删除。",
    "- 删除后续由{{user}}新增的校规不需要支付代价；成功只remove对应校规，不返还当前MC点、MC能量或金钱。",
    "</校规规则>"
  ].join("\n");
  ...
  patchEntryContent(entries, "[mvu_plot]强调要求", (content) => {
    ...
    if (!next.includes(bodyStatsLine)) {
      next = next.replace(
	    ...
	    const resourceGlossaryLine = "- 资源名必须严格区分：`系统.MC能量`是催眠功能实际消耗的能量余额；`系统.累计消耗MC点`是历史消耗/解锁阈值，不可花费；`系统.MC能量上限`只是能量容量上限，不是可花费余额；`系统.当前MC点`是PT/MC点货币，不等于MC能量，不能替代能量支付。";
  ...
  patchEntryContent(entries, "[mvu_plot]催眠指导", (content) =>
    ensureLinesBeforeEndTag(
      ...
      [
	        "- 新前端会在启动催眠和每条催眠功能里用中文字段写明`预计消耗`、`MC能量消耗`、`是否受人数影响`、`是否受时间影响`、`人数`和`时间`；AI应优先按这些字段结算。群体类命令本身不因人数额外加价，永久/一次性命令不因时间额外加价。",
  ...
  patchEntryContent(entries, "[mvu_plot]日历和日程表*EJS制作中", (content) => {
    const scheduleBlock = [
      ...
      "  - 前端会把计算出的`当天课程表`、`当前/待上课程`和`当前或下个特殊日期`写入MVU变量；AI不要手动维护这些字段。",
      "  - AI叙事和变量更新应维护`当前日期`、`当前时间`、`当前日程`和`当前事件`，并检查当前场景是否符合课程/周末/假期。",
      "  - `当前/待上课程`由前端按普通课表显示当前正在上的课程或最近一节待上课程；没有课程、周末、假期、考试或活动日写`无`。",
  ...
  patchEntryContent(entries, "[mvu_update]变量说明和更新规则🈯", (content) => {
    const resourceRuleBlock = [
	      ...
	      "        - 成功扣除MC能量时，还必须同步把`/系统/累计消耗MC点`增加同样数值。",
	      "        - 日期推进到新一天/跨日时，MC能量自然恢复`MC能量上限`的一半；跨多天按天数累计，最终不得超过`MC能量上限`。若每日结算脚本已经恢复过，不要重复恢复。",
	      "    累计消耗MC点:",
	      "      type: number",
	      "      info: 历史累计消耗的MC能量点数，用于VIP解锁阈值；不是当前可花费余额。",
	      "      check:",
	      "        - 每次启动/追加催眠成功并实际扣除MC能量时，同步增加相同数值。",
	      "        - 订阅VIP等级时可作为判断该等级是否达到解锁阈值的依据；它不能支付费用，也不能替代`当前MC点`或`MC能量`。",
	      "        - 催眠失败、余额不足、订阅、补给、购买当前MC点、提升能量上限时不要增加这个值。",
	      "    MC能量上限:",
	      "      type: number",
      ...
      "    当前MC点:",
      "      type: number",
	      "      info: PT/MC点货币；可通过成就、任务或高潮奖励获得，消费用途仅限提升`MC能量上限`；与`MC能量`完全分离。",
      "      check:",
      "        - 被催眠角色每个部位每高潮一次获得5点",
      "        - 快感值大于200的高潮获得10点, 大于300获得20, 大于400获得40点, 大于500获得80点",
	      "        - 成就领取或任务完成时根据说明/`奖励MC点`直接获得，并在同一次<update>中写入待前端同步的已完成成就/任务记录。",
	      "        - 只有提升`MC能量上限`需要支付当前MC点；启动/追加催眠、订阅和补充MC能量都不消耗当前MC点。",
	      "        - 重要：**只有**获得成就奖励、任务奖励、高潮奖励或提升能量上限时当前MC点才会更新；其他情况永远不要更新当前MC点。"
    ].join("\n");
    ...
    let next = content
      .replace(
	    ...
	    const scheduleVariableBlock = [
	      "    当前/待上课程:",
	      "      check:",
	      "        - 由前端根据`当前日期`、`当前时间`和内置周课表自动维护；AI不要手动维护。",
	      "        - 普通授课中写具体科目，课前/课间写最近一节待上课程；无课程、周末、假期、考试或特别活动时写`无`。",
	      "    当天课程表:",
	      "      type: object",
	      "      check:",
	      ...
	      "    当前或下个特殊日期:",
	      "      check:",
	      "        - 由前端根据`当前日期`和日历特殊日期自动维护，显示当前或之后的下一个特殊日期；AI不要手动维护。",
	      "    当前事件:",
	      "      check:",
	      "        - 写当前最具体的事件/场景，可包含课程、午休、考试、节日、社团招新、体育祭、放学后安排或剧情事件；用于AI理解当前舞台。"
	    ].join("\n");
	    next = next.replaceAll("当前或下个事件", "当前或下个特殊日期");
	    if (!next.includes("    当前/待上课程:")) {
	      next = next.replace(/    当前或下个特殊日期:\n[\s\S]*?(?=\n    学校声望:)/, scheduleVariableBlock);
	    }
    next = next.replace(
    ...
    const mcEnergyPatchLine = "        - 若本轮APP操作中的催眠功能成功且有`MC能量消耗`，必须输出JSON Patch：`{ \"op\": \"replace\", \"path\": \"/系统/MC能量\", \"value\": 当前系统.MC能量 - 实际MC能量消耗 }`，并同步把`/系统/累计消耗MC点`增加同样数值；失败则不要扣。";
  ...
  patchEntryContent(entries, "[mvu_update]本轮APP操作", (content) => {
    const next = [
      ...
      "- 如果本轮用户输入中没有<本轮APP操作>容器，或容器为空/无，则代表{{user}}没有操作APP，严禁进行相关新增操作描写。",
	      ...
	      "- 资源名必须严格区分：`MC能量`=催眠能量余额；`累计消耗MC点`=历史消耗/解锁阈值；`MC能量上限`=容量上限，不可花费；`当前MC点`=PT/MC点货币；`持有零花钱`=金钱。不同资源不能互相顶替。",
		      "- `本轮APP操作`最外层可能包含一次`<相关变量>`：它不是MVU字段，也不写入MVU；只汇总本批操作会检查、增加或减少的变量，避免每条操作重复携带。",
		      "- 相关变量含义：启动/追加催眠给`MC能量`和`累计消耗MC点`；订阅给`持有零花钱`、`催眠APP订阅等级`和`累计消耗MC点`；补充MC能量给`持有零花钱`、`MC能量`、`MC能量上限`；购买当前MC点给`持有零花钱`和`当前MC点`；提升MC能量上限给`当前MC点`和`MC能量上限`；领取成就/任务奖励给`当前MC点`；申请/废止初始校规则给`当前MC点`、订阅等级、爱丽莎好感度和当前校规数。",
		      "- 同一批次里若先奖励获得`当前MC点`、后又消耗`当前MC点`提升能量上限，AI应按`本轮APP操作`中的操作顺序，从`<相关变量>`初始值开始逐项结算；未出现在`<相关变量>`中的资源不要自行脑补为可用。",
      ...
      "- 催眠APP启动/追加催眠会携带总`预计消耗`、`MC能量消耗`，并在每项功能里携带`是否受人数影响`与`是否受时间影响`；AI结算时优先按这些中文字段处理。标记为不受人数影响的群体类命令不按人数乘算，标记为不受时间影响的永久/一次性命令不按持续时间乘算。",
      "- 若催眠功能成功并产生`MC能量消耗`，必须用JSON Patch更新`/系统/MC能量`为扣除后的余额，并把`/系统/累计消耗MC点`增加同样数值；若余额不足或操作失败，则两者都不得更新。",
      ...
      "- 催眠APP、领取任务、完成成就、订阅、补给、库存、日历、扫描角色、删除角色、新增任务、地图/学校地图地点建议和申请立校规等操作都按本规则结算；校规的作用范围与写入位置见[mvu_update]校规规则。",
  ...
  patchEntryContent(entries, "[mvu_update]变量更新格式", (content) => {
    const updatePolicyLines = [
      ...
      "    - 中文结算要求：成功的催眠APP操作如果有`MC能量消耗`，必须同时写`/系统/MC能量`扣除和`/系统/累计消耗MC点`增加；不能只更新`当前MC点`、只更新`累计消耗MC点`或漏掉`MC能量`。",
      "    - 日期推进到新一天/跨日时，若当前变量尚未由每日结算脚本恢复，则`/系统/MC能量`恢复`MC能量上限`的一半并封顶到`MC能量上限`。",
