...
function renderPhoneCourseTimetable() {
  const days = [
    ["周一", "现代文", "数学II", "英语会话", "日本史", "体育", "家庭科"],
    ["周二", "古典", "化学基础", "数学II", "英语阅读", "美术", "LHR"],
    ["周三", "英语阅读", "世界史", "生物基础", "现代文", "体育", "信息"],
    ["周四", "数学II", "古典", "英语会话", "化学基础", "音乐", "家庭科"],
    ["周五", "现代文", "日本史", "生物基础", "英语阅读", "数学演习", "综合探究"]
  ];
  const periods = [
    ["1限", "08:40-09:30"],
    ["2限", "09:40-10:30"],
...
function patchOriginalWorldbookForMergedPolicy(entries) {
  ...
  patchEntryContent(entries, "[mvu_plot]日历和日程表*EJS制作中", (content) => {
    const scheduleBlock = [
      ...
      "",
      "[Weekly Timetable]",
      "星期一: 1限现代文 / 2限数学II / 3限英语会话 / 4限日本史 / 5限体育 / 6限家庭科",
      "星期二: 1限古典 / 2限化学基础 / 3限数学II / 4限英语阅读 / 5限美术 / 6限LHR",
      "星期三: 1限英语阅读 / 2限世界史 / 3限生物基础 / 4限现代文 / 5限体育 / 6限信息",
      "星期四: 1限数学II / 2限古典 / 3限英语会话 / 4限化学基础 / 5限音乐 / 6限家庭科",
      "星期五: 1限现代文 / 2限日本史 / 3限生物基础 / 4限英语阅读 / 5限数学演习 / 6限综合探究",
      ...
      "  - AI叙事和变量更新应维护`当前日期`、`当前时间`、`当前日程`，并检查当前场景是否符合课程/周末/假期。"
    ].join("\n");
  ...
  patchEntryContent(entries, "[mvu_update]变量说明和更新规则🈯", (content) => {
    const resourceRuleBlock = [
      ...
      "        - 任务完成时根据任务说明或`奖励MC点`直接获得，并在同一次<update>中删除该任务",
      ...
      "        - 只有当本轮剧情明确展示或改变角色当下想法时更新；内容应贴合当前场景、目标和短期注意力。",
	    ...
	    const taskRuleBlock = [
	      ...
	      "      }",
	      "    check:",
	      "      - `任务`变量只保存已接/进行中的任务，最多3个；静态任务未接取前不写入变量。",
	      "      - 新增任务直接写入这里，必须包含`完成条件`、`奖励MC点`和`已完成:false`；若已有进行中任务为3个，则不得新增。",
	      "      - 当本轮回复满足任务完成条件时，直接把奖励加到`系统/当前MC点`，并在同一次<update>中`remove`该任务。",
	      "      - 不要把完成的任务保留为`已完成:true`，也不要保存已完成任务列表；任务完成即领奖并从变量中消失。"
	    ].join("\n");
    let next = content
      .replace(
  ...
  patchEntryContent(entries, "[mvu_update]本轮APP操作", (content) => {
    const next = [
      ...
      "- `任务`变量只保存已接/进行中的任务，最多3个；新增任务操作表示用户希望AI根据当前上下文剧情直接生成并接取若干任务，数量不得超过`3-当前已接任务数`，若已接任务数为3则本次新增失败。",
      "- 新增任务必须写入`/任务/任务名`，包含`完成条件`、`奖励MC点`和`已完成:false`；不要写入前端静态列表，也不要新增为已完成任务。",
      "- 任务完成后不等待用户点击领取：若本轮剧情满足某个任务完成条件，AI必须在同一次<update>中把奖励加到`/系统/当前MC点`，并`remove`对应`/任务/任务名`；不要把任务保留为`已完成:true`，也不要另存已完成任务列表。",
